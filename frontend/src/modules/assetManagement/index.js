...
define(function (require) {
    ...
    new TagsCollection().fetch({
      success: function (tagsCollection) {
        ...
        Origin.sidebar.addView(
          new AssetManagementSidebarView({ collection: tagsCollection }).$el
        );
        Origin.contentPane.setView(AssetManagementView, {
  ...
  function loadNewAssetView() {
    Origin.trigger('location:title:update', {
      title: Origin.l10n.t('app.newasset'),
    });
    Origin.sidebar.addView(new AssetManagementNewAssetSidebarView().$el);
    Origin.contentPane.setView(AssetManagementNewAssetView, {
  ...
  function loadEditAssetView(location) {
    ...
    new AssetModel({ _id: location }).fetch({
      success: function (model) {
        Origin.trigger('location:title:update', {
          title: Origin.l10n.t('app.editasset'),
        });
        Origin.sidebar.addView(new AssetManagementNewAssetSidebarView().$el);
        Origin.contentPane.setView(AssetManagementNewAssetView, {
          model: model,
