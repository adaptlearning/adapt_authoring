...
define(function (require) {
  ...
  var AssetManagementModalFiltersView = Backbone.View.extend({
    ...
    events: {
      ...
      'click .asset-management-modal-filter-clear-search':
    ...
    initialize: function (options) {
      ...
      this.listenTo(
        Origin,
        'assetManagement:modalTags:filterByTags',
    ...
    onClearSearchClicked: function (event) {
      ...
      this.$('.asset-management-modal-filter-search')
        .val('')
        .trigger('keydown', [true]);
    },
    ...
    onAddTagClicked: function (event) {
      ...
      if (!exists) {
        $('.modal-popup-toolbar').after(
          new AssetManagementModalTagsView({
    ...
    filterAssetsByTags: function (tag) {
      ...
      Origin.trigger(
        'assetManagement:assetManagementSidebarView:filterByTags',
        this.usedTags
      );
    },
    ...
    onAddAssetClicked: function (event) {
      ...
      $('.modal-popup-content').append(
        new AssetManagementModalNewAssetView({ model: new AssetModel() }).$el
      );
    },
  });
