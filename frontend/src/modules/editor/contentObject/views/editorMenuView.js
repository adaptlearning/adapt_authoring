  ...
  var EditorMenuView = EditorOriginView.extend(
    {
      ...
      renderLayers: function () {
        var selectedModel =
          Origin.editor.currentContentObject || Origin.editor.data.course;
      ...
      updateItemViews: function (previousParent, model) {
        ...
        for (
          var i = 0;
          i < this.layerViews[previousParent].childViews.length;
          i++
        ) {
      ...
      getItemHeirarchy: function (model, done) {
        ...
        var _getParent = function (model, callback) {
          var parent = __this.contentobjects.findWhere({
        ...
        _getParent(model, function () {
          if (typeof done === 'function')
            done.call(__this, hierarchy.reverse());
        });
      },
      ...
      onSelectedItemChanged: function (model) {
        if (
          model &&
          model.get('_id') === Origin.editor.currentContentObject &&
          Origin.editor.currentContentObject.get('_id')
        )
          return;
      ...
      scrollToElement: function () {
        ...
        this.$('.editor-menu-layer-inner').scrollTo('.expanded, .selected', {
          duration: 300,
          offset: { top: -20, left: 0 },
          axis: 'y',
        });
        this.$el.scrollTo($('.selected').closest('.editor-menu-layer'), {
          duration: 300,
          axis: 'x',
        });
      },
      ...
      setupDragDrop: function () {
        $('.editor-menu-layer-inner').sortable({
          ...
          stop: _.bind(function (event, ui) {
            ...
            var id = $('.editor-menu-item-inner', $draggedElement).attr(
              'data-id'
            );
            var sortOrder = $draggedElement.index() + 1;
            var parentId = $draggedElement
              .closest('.editor-menu-layer')
              .attr('data-parentId');
      ...
      onItemDeleted: function (oldModel) {
        this.contentobjects.fetch({
          success: _.bind(function () {
            ...
            Origin.trigger(
              'editorView:menuView:updateSelectedItem',
              this.contentobjects.findWhere({ _id: oldModel.get('_parentId') })
            );
          }, this),
          error: function () {
            Origin.Notify.alert({
