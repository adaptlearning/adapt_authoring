  ...
  var EditorPageBlockView = EditorOriginView.extend(
    {
      ...
      preRender: function () {
        ...
        this.model.set(
          'componentTypes',
          Origin.editor.data.componenttypes.toJSON()
        );
        this.render();
      },
      ...
      handleAsyncPostRender: function () {
        ...
        this.listenTo(
          Origin,
          'editorPageComponent:postRender',
          function (view) {
            var id = view.model.get('_id');
            if (
              this.children.indexOf(view.model) !== -1 &&
              renderedChildren.indexOf(id) === -1
            ) {
      ...
      listenToEvents: function () {
        ...
        var events = {
          'editorView:removeSubViews editorPageView:removePageSubViews':
            this.remove,
        };
        events[
          'editorView:addComponent:' +
            id +
            ' ' +
            'editorView:removeComponent:' +
            id +
            ' ' +
            'editorView:moveComponent:' +
            id
        ] = this.render;
      ...
      deleteBlockPrompt: function (event) {
        ...
        Origin.Notify.confirm({
          ...
          text:
            Origin.l10n.t('app.confirmdeleteblock') +
            '<br />' +
            '<br />' +
      ...
      setupDragDrop: function () {
        ...
        this.$el.draggable({
          ...
          helper: function (e) {
            ...
            view.offsetTopFromWindow =
              view.$el.offset().top - $(window).scrollTop();
            ...
            view.showDropZones();
            $(this).attr(
              'data-' + view.model.get('_type') + '-id',
              view.model.get('_id')
            );
            $(this).attr(
              'data-' + view.model.get('_parent') + '-id',
              view.model.get('_parentId')
            );
            return $(
              '<div class="drag-helper">' + view.model.get('title') + '</div>'
            );
          },
          start: function (event) {
            // Using the initial offset we're able to position the window back in place
            $(window).scrollTop(
              view.$el.offset().top - view.offsetTopFromWindow
            );
          },
          drag: function (event) {
            ...
            if (clientY < offsetTop + SCROLL_THRESHOLD) {
              ...
              clientY >
              $container.height() + offsetTop - SCROLL_THRESHOLD
            ) {
              scrollAmount = SCROLL_INCREMENT;
            }
      ...
      addComponentViews: function () {
        ...
        var addPasteZonesFirst =
          this.children.length && this.children[0].get('_layout') !== 'full';
      ...
      loadBlockEdit: function (event) {
        ...
        var id = this.model.get('_id');
        Origin.router.navigateTo(
          'editor/' + courseId + '/' + type + '/' + id + '/edit'
        );
      },
      ...
      showComponentList: function (event) {
        ...
        $('body').append(
          new EditorPageComponentListView({
      ...
      setupPasteZones: function () {
        ...
        _.each(
          this.sortArrayByKey(dragLayouts, 'pasteZoneRenderOrder'),
          function (layout) {
            ...
            var $pasteEl = new EditorPageComponentPasteZoneView({
              model: pasteComponent,
            }).$el;
        ...
        _.each(
          this.sortArrayByKey(layouts, 'pasteZoneRenderOrder'),
          function (layout) {
            var pasteComponent = new ComponentModel();
            pasteComponent.set('_parentId', this.model.get('_id'));
            pasteComponent.set('_type', 'component');
            ...
            this.$('.page-components').append(
              new EditorPageComponentPasteZoneView({ model: pasteComponent })
                .$el
            );
          },
          this
