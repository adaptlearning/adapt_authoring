  ...
  var EditorPageComponentView = EditorOriginView.extend(
    {
      ...
      deleteComponentPrompt: function (event) {
        ...
        Origin.Notify.confirm({
          ...
          text:
            Origin.l10n.t('app.confirmdeletecomponent') +
            '<br />' +
            '<br />' +
            Origin.l10n.t('app.confirmdeletecomponentwarning'),
      ...
      deleteComponent: function () {
        this.model.destroy({
          success: _.bind(function (model) {
            this.remove();
            Origin.trigger(
              'editorView:removeComponent:' + model.get('_parentId')
            );
          }, this),
      ...
      getSupportedLayout: function () {
        var componentType = Origin.editor.data.componenttypes.findWhere({
          component: this.model.get('_component'),
        });
      ...
      evaluateLayout: function (cb) {
        ...
        this.model.fetchSiblings(
          _.bind(function (siblings) {
      ...
      evaluateMove: function (event) {
        ...
        this.model.fetchSiblings(
          _.bind(function (siblings) {
            ...
            this.moveComponent(
              this.model.get('_id'),
              isLeft ? 'left' : isRight ? 'right' : 'full'
            );
            // move sibling to inverse of self
            var siblingId =
              siblings && siblings.length > 0 && siblings.models[0].get('_id');
            if (siblingId)
              this.moveComponent(siblingId, isLeft ? 'right' : 'left');
          }, this)
        );
      },
