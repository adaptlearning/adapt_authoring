...
define(function (require) {
  ...
  function renderCourseEdit() {
    ...
    CoreHelpers.multiModelFetch(
      [courseModel, Origin.editor.data.config],
      function (data) {
        EditorHelpers.setPageTitle(courseModel);
        var form = Origin.scaffold.buildForm({ model: courseModel });
        Origin.contentPane.setView(EditorCourseEditView, {
          model: courseModel,
          form: form,
        });
        Origin.sidebar.addView(
          new EditorCourseEditSidebarView({ form: form }).$el
        );
      }
    );
  }
