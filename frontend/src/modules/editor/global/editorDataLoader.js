...
define(function (require) {
  ...
  var preloader = {
    ...
    loadGlobalData: function () {
      ...
      for (var collName in globalData) {
        if (
          globalData.hasOwnProperty(collName) &&
          !Origin.editor.data[collName]
        ) {
          Origin.editor.data[collName] = createCollection(
            globalDataMap[collName]
          );
        }
      }
      // start preload
    ...
    loadCourseData: function (callback) {
      ...
      var isAlreadyLoaded =
        preloader.hasLoadedCourseData() ||
        (ed.course && ed.course.get('_id') === courseId);
  ...
  function createCollection(Model, query) {
    ...
    if (siblingTypes !== undefined) {
      if (!courseId)
        throw new Error('No Editor.data.course specified, cannot load ' + url);
