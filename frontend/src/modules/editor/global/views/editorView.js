  ...
  var EditorView = EditorOriginView.extend(
    {
          ...
          function (data, textStatus, jqXHR) {
            if (!data.success) {
              ...
              Origin.Notify.alert({
                type: 'error',
                text:
                  Origin.l10n.t('app.errorgeneratingpreview') +
      ...
      exportProject: function (error) {
        ...
        $.ajax({
          ...
          success: function (data, textStatus, jqXHR) {
            ...
            form.setAttribute(
              'action',
              'export/' + tenantId + '/' + courseId + '/download.zip'
            );
            form.submit();
          },
          error: function (jqXHR, textStatus, errorThrown) {
            ...
            Origin.Notify.alert({
              ...
              text:
                Origin.l10n.t('app.errorgeneric') +
                Origin.l10n.t('app.debuginfo', {
                  message: jqXHR.responseJSON.message,
                }),
            });
      ...
      showExportAnimation: function (show, $btn) {
        if (show !== false) {
          $('.editor-common-sidebar-export-inner', $btn).addClass(
            'display-none'
          );
          $('.editor-common-sidebar-exporting', $btn).removeClass(
            'display-none'
          );
        } else {
          $('.editor-common-sidebar-export-inner', $btn).removeClass(
            'display-none'
          );
        ...
        $.get(
          ...
          function (data, textStatus, jqXHR) {
            ...
            $downloadForm.attr(
              'action',
              'download/' +
                Origin.sessionModel.get('tenantId') +
                '/' +
                Origin.editor.data.course.get('_id') +
                '/' +
                data.payload.zipName +
                '/download.zip'
            );
            $downloadForm.submit();
          }.bind(this)
      ...
      resetPreviewProgress: function () {
        ...
        $('.editor-common-sidebar-preview-wrapper .dropdown').removeClass(
          'active'
        );
        Origin.editor.isPreviewPending = false;
      },
