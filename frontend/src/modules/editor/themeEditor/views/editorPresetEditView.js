  ...
  var PresetEditView = Backbone.View.extend(
    {
      ...
      onSaveClicked: function (event) {
        ...
        if (newValue === '') {
          $preset
            .find('.preset-error')
            .text(Origin.l10n.t('app.required'))
            .removeClass('display-none');
          return;
        }

        var theme = $('.theme select').val();
        var presets = this.model
          .get('presets')
          .where({ parentTheme: theme, displayName: newValue });
        if (presets.length > 0) {
          $preset
            .find('.preset-error')
            .text(Origin.l10n.t('app.duplicatepreseterror'))
            .removeClass('display-none');
          return;
        }
      ...
      onDeleteClicked: function (event) {
        event && event.preventDefault();
        var self = this;
        var presetName = $(event.currentTarget)
          .closest('.preset')
          .attr('data-name');
