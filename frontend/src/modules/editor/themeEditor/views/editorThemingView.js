  ...
  var ThemingView = EditorOriginView.extend(
    {
      ...
      initialize: function () {
        ...
        this.listenTo(Origin, {
          ...
          'editorThemingSidebar:views:resetToPreset':
            this.restorePresetSettings,
      ...
      render: function () {
        ...
        Origin.trigger('location:title:update', {
          breadcrumbs: [
            'dashboard',
            'course',
            { title: Origin.l10n.t('app.themeeditor') },
          ],
      ...
      renderForm: function () {
        ...
        if (
          selectedTheme.get('name') ===
            Origin.editor.data.config.get('_theme') &&
          Origin.editor.data.course.get('themeVariables')
        ) {
      ...
      updatePresetSelect: function () {
        ...
        select.append(
          $('<option>', { value: '', selected: 'selected' }).text(
            Origin.l10n.t('app.nopresets')
          )
        );
        // add options
        presets.forEach(function (item) {
          select.append(
            $('<option>', { value: item.get('_id') }).text(
              item.get('displayName')
            )
          );
        }, this);
        ...
        if (selectedPreset && selectedPreset.get('parentTheme') === theme) {
          $.get(
            'api/themepreset/exists/' + selectedPreset.get('_id'),
            function (data) {
              if (data.success) {
        ...
        for (var key in toRestore) {
          ...
          if (typeof toRestore[key] === 'object') {
            for (var innerKey in toRestore[key]) {
              this.restoreField(
                this.form.fields[innerKey],
                toRestore[key][innerKey],
                innerKey
              );
            }
          } else {
      ...
      restoreField: function (fieldView, value, key) {
        ...
        var inputType =
          fieldView.schema.inputType.type || fieldView.schema.inputType;
      ...
      showPresetEdit: function (event) {
        ...
        var pev = new PresetEditView({
          model: new Backbone.Model({
            presets: new Backbone.Collection(
              this.presets.where({ parentTheme: parentTheme })
            ),
          }),
        });
        $('body').append(pev.el);
        ...
        presetModel.save(null, {
          ...
          success: function () {
            ...
            window.setTimeout(function () {
              self.$('.preset select').val(presetModel.get('_id'));
            }, 1);
          },
        });
      },
      ...
      getSelectedPreset: function (includeCached) {
        ...
        if (includeCached !== false) {
          ...
          return this.presets.findWhere({
            _id: this.model.get('_themePreset'),
            parentTheme: parent,
          });
        }
      },
      ...
      updateRestorePresetButton: function (shouldShow) {
        if (typeof shouldShow === 'undefined') {
          ...
          var currentSettings = this.flattenNestedProperties(
            this.getCurrentSettings()
          );
          var preset = this.getSelectedPreset();
          var baseSettings = this.flattenNestedProperties(
            preset ? preset.get('properties') : this.getDefaultThemeSettings()
          );
      ...
      onPresetChanged: function (event) {
        var preset = this.presets.findWhere({
          _id: $(event.currentTarget).val(),
        });
        var settings =
          (preset && preset.get('properties')) ||
      ...
      onSavePresetClicked: function () {
        ...
        Origin.Notify.alert({
          ...
          callback: function (presetName) {
            ...
            if (presetName === '')
              return swal.showInputError(Origin.l10n.t('app.invalidempty'));
            var theme = self.$('.theme select').val();
            var presets = self.presets.where({
              parentTheme: theme,
              displayName: presetName,
            });
            if (presets.length > 0) {
