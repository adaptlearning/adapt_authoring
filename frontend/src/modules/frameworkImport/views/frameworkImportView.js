  ...
  var FrameworkImportView = OriginView.extend(
    {
      ...
      showDetails: function (sidebarView) {
        ...
        this.sidebarView.updateButton(
          '.framework-import-sidebar-save-button',
          Origin.l10n.t('app.importing')
        );
      ...
      displayDetails: function (data) {
        ...
        if (
          _.isEmpty(data.pluginVersions.red) &&
          !data.frameworkVersions.downgrade
        ) {
        ...
        if (
          data.frameworkVersions.imported !==
            data.frameworkVersions.installed &&
          !data.frameworkVersions.downgrade
        ) {
          $frameworkVersions
            .html(
              Origin.l10n.t('app.importframeworkversions', {
      ...
      displaySummary: function (data) {
        ...
        $summaryTitle
          .addClass('red')
          .text(Origin.l10n.t('app.coursecannotbeimported'));

        if (!_.isEmpty(data.pluginVersions.red)) {
          $summaryDescription.text(
            Origin.l10n.t('app.coursecannotbeimporteddesc')
          );
          return;
        }
        ...
        if (data.frameworkVersions.downgrade) {
          $summaryDescription.text(
            Origin.l10n.t('app.coursecannotbeimporteddowngradedesc')
          );
          return;
        }
        ...
        $summaryTitle
          .removeClass('red')
          .text(Origin.l10n.t('app.coursecanbeimported'));

        var greenExists = !(
          _.isEmpty(data.pluginVersions['green-install']) &&
          _.isEmpty(data.pluginVersions['green-update'])
        );
        if (_.isEmpty(data.pluginVersions.amber) && !greenExists) {
          $summaryDescription.text(
            Origin.l10n.t('app.coursecanbeimportedwhitedesc')
          );
          return;
        }
      ...
      displayPluginList: function (data) {
        ...
        for (var i = 0, j = categories.length; i < j; i++) {
          ...
          pluginArray.forEach(function (plugin) {
            ...
            $pluginList
              .find('.frameworkImportPlugin-plugins')
              .append(new FrameworkImportPluginView({ data: plugin }).$el);
          });

          $pluginList.removeClass('display-none');
          $pluginList
            .find('.key-field.' + categories[i])
            .removeClass('display-none');
        }
      },
      ...
      onAjaxError: function (data, status, error) {
        ...
        var msg =
          (resJson.body && resJson.body.replace(/\n/g, '<br />')) || error;
