  ...
  var PluginTypeView = OriginView.extend(
    {
      ...
      toggleAddedDefault: function () {
        this.model.save(
          {
            _isAddedByDefault: this.$('.pluginType-addedDefault').is(
              ':checked'
            ),
          },
          { patch: true }
      ...
      checkForUpdates: function (event) {
        ...
        $.get(
          this.model.urlRoot + '/checkversion/' + this.model.get('_id'),
          function (data) {
            ...
            $btn
              .attr({ title: Origin.l10n.t('app.updateplugin') })
              .removeClass('plugin-update-check')
              .addClass('plugin-update-confirm');
      ...
      updatePlugin: function (event) {
        ...
        $.post(this.model.urlRoot + '/update', {
          targets: [this.model.get('_id')],
        })
          .done(
            function () {
              Origin.trigger(
                'scaffold:updateSchemas',
                function () {
          ...
          function (data) {
            ...
            if (data.courses.length === 0) {
              Origin.Notify.confirm({
                ...
                text: Origin.l10n.t('app.confirmdeleteplugin', {
                  plugin: this.model.get('displayName'),
                }),
                destructive: false,
            ...
            for (var i = 0, len = data.courses.length; i < len; i++) {
              courses +=
                ...
                ' ' +
                Origin.l10n.t('app.by') +
                ' ' +
                data.courses[i].createdByEmail +
                '<br />';
            }
            popup.type = 'error';
            popup.title =
              Origin.l10n.t('app.cannotdelete') +
              ' ' +
              this.model.get('displayName');
            popup.text =
              Origin.l10n.t('app.coursesused') + '<br />' + courses + '<br />';
