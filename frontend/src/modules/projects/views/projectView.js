  ...
  var ProjectView = OriginView.extend(
    {
      ...
      events: {
        ...
        'click .projects-details-tags-button-show':
          'onProjectShowTagsButtonClicked',
        'click .projects-details-tags-button-hide':
          'onProjectHideTagsButtonClicked',
      },
      ...
      preRender: function () {
        ...
        this.listenTo(
          Origin,
          'editorView:deleteProject:' + this.model.get('_id'),
          this.deleteProject
        );
      ...
      editProjectSettings: function (event) {
        ...
        Origin.router.navigateTo(
          'editor/' + this.model.get('_id') + '/settings'
        );
      },
      ...
      deleteProjectPrompt: function (event) {
        event && event.preventDefault();
        var isShared =
          this.model.get('_isShared') ||
          (this.model.get('_shareWithUsers') &&
            this.model.get('_shareWithUsers').length > 0);
        var titleKey = isShared ? 'deletesharedproject' : 'deleteproject';
        var textKey = isShared
          ? 'confirmdeletesharedprojectwarning'
          : 'confirmdeleteprojectwarning';

        Origin.Notify.confirm({
          type: 'warning',
          title: Origin.l10n.t('app.' + titleKey),
          text:
            Origin.l10n.t('app.confirmdeleteproject') +
            '<br/><br/>' +
            Origin.l10n.t('app.' + textKey),
      ...
      duplicateProject: function () {
        $.ajax({
          ...
          success: function (data) {
            Origin.router.navigateTo(
              'editor/' + data.newCourseId + '/settings'
            );
          },
          error: function () {
            Origin.Notify.alert({
