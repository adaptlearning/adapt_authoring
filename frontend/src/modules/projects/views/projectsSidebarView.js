  ...
  var ProjectsSidebarView = SidebarItemView.extend(
    {
      ...
      postRender: function () {
        this.listenTo(
          Origin,
          'sidebarFilter:filterByTags',
          this.filterProjectsByTags
        );
        this.listenTo(
          Origin,
          'sidebarFilter:addTagToSidebar',
          this.addTagToSidebar
        );
      ...
      highlightSearchBox: function () {
        this.$('.projects-sidebar-filter-search-input').removeClass(
          'search-highlight'
        );
        if (this.$('.projects-sidebar-filter-search-input').val()) {
          this.$('.projects-sidebar-filter-search-input').addClass(
            'search-highlight'
          );
        }
      },
      ...
      updateUI: function (userPreferences) {
        if (userPreferences.search) {
          this.$('.projects-sidebar-filter-search-input').val(
      ...
      filterProjectsByTitle: function (event, filter) {
        ...
        Origin.trigger(
          'dashboard:dashboardSidebarView:filterBySearch',
          filterText
        );
        this.highlightSearchBox();
      },
      ...
      clearFilterInput: function (event) {
        ...
        $currentTarget
          .prev('.projects-sidebar-filter-input')
          .val('')
          .trigger('keyup', [true]);
        this.highlightSearchBox();
      },
      ...
      onAddTagClicked: function (event) {
        ...
        this.collection.each(function (tag) {
          ...
          if (
            !_.contains(availableTagsTitles, tag.get('title')) &&
            !_.contains(usedTagTitles, tag.get('title'))
          ) {
