  ...
  var ProjectsView = OriginView.extend(
    {
      ...
      initEventListeners: function () {
        ...
        this.listenTo(Origin, {
          ...
          'dashboard:dashboardSidebarView:filterBySearch': function (text) {
            this.doFilter(text);
          },
          'dashboard:dashboardSidebarView:filterByTags': function (tags) {
            this.doFilter(null, tags);
          },
          'dashboard:sort:asc': function () {
            this.doSort('asc');
          },
          'dashboard:sort:desc': function () {
            this.doSort('desc');
          },
          'dashboard:sort:updated': function () {
            this.doSort('updated');
          },
        });

        this.supportedLayouts.forEach(function (layout) {
          this.listenTo(Origin, 'dashboard:layout:' + layout, function () {
      ...
      initUserPreferences: function () {
        ...
        $("a[data-callback='dashboard:layout:" + prefs.layout + "']").addClass(
          'selected'
        );
        $("a[data-callback='dashboard:sort:" + prefs.sort + "']").addClass(
          'selected'
        );
      ...
      getUserPreferences: function () {
        var prefs = OriginView.prototype.getUserPreferences.apply(
          this,
          arguments
        );
      ...
      appendProjectItem: function (model) {
        var creator = model.get('createdBy') || {
          email: Origin.l10n.t('app.unknownuser'),
        };
        var name = creator.firstName
          ? creator.firstName + ' ' + creator.lastName
          : creator.email;
        if (this._isShared && name) model.set('creatorName', name);
        this.getProjectsContainer().append(
          new ProjectView({ model: model }).$el
        );
      },
      ...
      doLazyScroll: function (e) {
        ...
        var pxRemaining =
          this.getProjectsContainer().height() -
