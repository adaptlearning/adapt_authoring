define(['core/origin', './models/schemasModel'], function (
  Origin,
  SchemasModel
) {
  var editorData;
  ...
  function trimPlugins(schema) {
    trimDisabledPlugins(
      schema._extensions,
      _.values(configModel.get('_enabledExtensions')),
      'targetAttribute'
    );
    ...
    ['menu', 'theme'].forEach(function (type) {
      ...
      if (current)
        trimDisabledPlugins(schema[type + 'Settings'], [current.toJSON()]);
    });
  }
  ...
  function trimGlobals(schema) {
    ...
    trimDisabledPlugins(
      globals._menu,
      editorData.menutypes.where({ name: configModel.get('_menu') })
    );
    trimDisabledPlugins(
      globals._theme,
      editorData.themetypes.where({ name: configModel.get('_theme') })
    );
  ...
  function trimDisabledPlugins(schemaData, enabledPlugins, propertyToCheck) {
    ...
    var enabledPluginNames = enabledPlugins.map(function (plugin) {
      return (
        plugin[propertyToCheck] || (plugin.get && plugin.get(propertyToCheck))
      );
    });
    Object.keys(schemaData.properties).forEach(function (schemaKey) {
      if (enabledPluginNames.indexOf(schemaKey) === -1)
        delete schemaData.properties[schemaKey];
    });
  }

  function trimEmptyProperties(object) {
    for (var key in object) {
      if (!object.hasOwnProperty(key) || object[key].type !== 'object')
        continue;
