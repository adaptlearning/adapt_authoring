  ...
  var ScaffoldAssetItemView = ScaffoldAssetView.extend(
    {
      ...
      findItemView: function () {
        ...
        switch (inputType.pluginType) {
          case 'extension':
            baseEditor =
              form.fields._extensions.editor.nestedForm.fields[targetAttr]
                .editor;
            break;
          case 'menu':
            baseEditor =
              form.fields.menuSettings.editor.nestedForm.fields[targetAttr]
                .editor;
            break;
          case 'theme':
            baseEditor =
              form.fields.themeSettings.editor.nestedForm.fields[targetAttr]
                .editor;
            break;
          default:
      ...
      updateDragItem: function (item) {
        ...
        item.$el
          .css({
            top:
              parseInt(item.view.value[this.topAttr]).toFixed(this.precision) +
              '%',
            left:
              parseInt(item.view.value[this.leftAttr]).toFixed(this.precision) +
              '%',
          })
          .text(index + 1);
      },
      ...
      onDragStop: function (event, ui) {
        ...
        var left = (
          parseInt($dragHandle.css('left')) /
          ($parent.width() / 100)
        ).toFixed(this.precision);
        var top = (
          parseInt($dragHandle.css('top')) /
          ($parent.height() / 100)
        ).toFixed(this.precision);
        var index = $dragHandle
      ...
      renderData: function (id) {
        ...
        this.$el.html(
          Handlebars.templates[this.constructor.template]({
            ...
            addLabel:
              this.schema.inputType.addLabel || Origin.l10n.t('app.add'),
          })
        );
