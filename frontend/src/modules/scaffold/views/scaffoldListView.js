define([
  ...
  var ScaffoldListView = Backbone.Form.editors.List.extend({
    ...
    render: function () {
      var instance = Backbone.Form.editors.__List.prototype.render.apply(
        this,
        arguments
      );
      // set-up drag 'n drop
  ...
  var ScaffoldListItemView = Backbone.Form.editors.List.Item.extend({
    events: function () {
      return _.extend(
        {},
        Backbone.Form.editors.__List.__Item.prototype.events,
        {
    ...
    cloneItem: function (event) {
      ...
      var parentId =
        parentAttributes._type === 'course'
          ? parentAttributes._id
          : parentAttributes._parentId;
  ...
  Origin.on('origin:dataReady', function () {
    ...
    // overrides
    Backbone.Form.editors.List.prototype.constructor.template =
      Handlebars.templates.list;
    Backbone.Form.editors.List.Item.prototype.constructor.template =
      Handlebars.templates.listItem;
    Backbone.Form.editors.List.Modal.prototype.itemToString = modalItemToString;

    Backbone.Form.editors.List.Modal.prototype.__openEditor =
      Backbone.Form.editors.List.Modal.prototype.openEditor;
  ...
  function modalItemToString(value) {
    ...
    return Object.keys(this.nestedSchema).reduce(
      function (parts, key) {
        var val = getModalItemValueString(value[key]);
        var title =
          this.nestedSchema[key].title ||
          Backbone.Form.Field.prototype.createTitle.call({ key: key });
        return (
          parts +
          '<p class="list-item-modal-item">' +
          wrapSchemaTitle(title) +
          val +
          '</p>'
        );
      }.bind(this),
  ...
  function openEditor() {
    ...
    Backbone.Form.editors.List.Modal.prototype.__openEditor.apply(
      this,
      arguments
    );
  }
  ...
  function getModalItemValueString(value) {
    ...
    if (Array.isArray(value)) {
      return Origin.l10n.t('app.items', {
        smart_count: Object.keys(value).length,
      });
    }
    ...
    for (var name in value) {
      if (value.hasOwnProperty(name))
        pairs += '<br />' + wrapSchemaTitle(name) + value[name];
    }
