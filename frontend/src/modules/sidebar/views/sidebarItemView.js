...
define(function (require) {
  ...
  var SidebarItemView = OriginView.extend({
    ...
    events: {
      ...
      'click .editor-common-sidebar-preview-wrapper .dropdown button':
        'toggleDropdown',
    },
    ...
    setupFieldsetFilters: function () {
      ...
      if (fieldsets.length > 0) {
        this.$('.sidebar-item-inner').append(
          Handlebars.templates['sidebarDivide']({ title: 'Filters' })
        );
      }
      _.each(
        fieldsets,
        function (fieldset) {
          this.$('.sidebar-item-inner').append(
            new SidebarFieldsetFilterView({
              model: new Backbone.Model(fieldset),
            }).$el
          );
        },
        this
            ...
            _.each(
              ...
              function (fieldset, key) {
                ...
                if (_.contains(fieldset.fields, attribute)) {
                  ...
                  // Set error message
                  this.$('.sidebar-fieldset-filter-' + className).addClass(
                    'error'
                  );
                }
              },
    ...
    updateButton: function (buttonClass, updateText) {
      this.$(buttonClass)
        .append(
          Handlebars.templates['sidebarUpdateButton']({
    ...
    resetButtons: function () {
      var $buttonsSpans = this.$('.sidebar-updating')
    ...
    navigateToEditorPage: function (page) {
      Origin.router.navigateTo(
        'editor/' + Origin.editor.data.course.get('_id') + '/' + page
      );
    },
