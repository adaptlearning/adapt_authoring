  ...
  var UserModel = Backbone.Model.extend({
    ...
    setTenantName: function (model, value, options) {
      ...
      if (typeof tenantId === 'string') {
        tenantName = model
          .get('globalData')
          .allTenants.findWhere({ _id: tenantId })
          .get('displayName');
        ...
        tenantName = model
          .get('globalData')
          .allTenants.findWhere({ _id: tenantId._id })
          .get('displayName');
      }
    ...
    setLockStatus: function (model, value, options) {
      var newLocked =
        this.get('failedLoginCount') >= Origin.constants.maxLoginAttempts;
