  ...
  var UserView = OriginView.extend(
    {
      ...
      onSaveClicked: function (event) {
        ...
        if (
          $input.val() &&
          this.model.get($input.attr('data-modelKey')) !== $input.val()
        ) {
      ...
      onSaveRoleClicked: function (event) {
        ...
        if (
          newRole &&
          this.model.get($input.attr('data-modelKey')) !== newRole
        ) {
          Helpers.ajax(
            'api/role/' + oldRole + '/unassign/' + this.model.get('_id'),
            null,
            'POST',
            function () {
              Helpers.ajax(
                'api/role/' + newRole + '/assign/' + self.model.get('_id'),
                null,
                'POST',
                function () {
                  self.model.fetch();
      ...
      onResetLoginsClicked: function () {
        ...
        Origin.Notify.confirm({
          text: Origin.l10n.t('app.confirmresetlogins', {
      ...
      onInviteClicked: function (e) {
        Origin.Notify.confirm({
          text: Origin.l10n.t('app.confirmsendinvite', {
            email: this.model.get('email'),
          }),
          callback: function (confirmed) {
            if (!confirmed) {
      ...
      onResetPasswordClicked: function (e) {
        Origin.Notify.confirm({
          text: Origin.l10n.t('app.confirmsendreset', {
            email: this.model.get('email'),
          }),
          callback: function (confirmed) {
            if (!confirmed) {
      ...
      onChangePasswordClicked: function () {
        ...
        Origin.Notify.confirm({
          ...
          text: Origin.l10n.t('app.resetpasswordinstruction', {
            email: this.model.get('email'),
          }),
          inputType: 'password',
          ...
          callback: function (newPassword) {
            ...
            Helpers.ajax(
              ...
              function () {
                ...
                Origin.Notify.alert({
                  type: 'success',
                  text: Origin.l10n.t('app.changepasswordtext', {
                    email: self.model.get('email'),
                  }),
                });
              }
        ...
        this.model.save(toSave, {
          ...
          error: function (model, response, options) {
            ...
            switch (errorCode) {
              ...
              case 11000:
                return self.onError(
                  Origin.l10n.t('app.duplicateuservalueerror', data)
                );
              default:
                return self.onError(
                  Origin.l10n.t('app.uservalueerror') +
                    ' (' +
                    errorMessage +
                    ')'
                );
