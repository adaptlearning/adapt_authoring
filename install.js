...
installHelpers.checkPrimaryDependencies(function (error) {
  ...
  installHelpers.getLatestFrameworkVersion(function (
    error,
    latestFrameworkTag
  ) {
    if (error) {
      return handleError(
        error,
        1,
        'Failed to get the latest framework version. Check package.json.'
      );
    }
    inputData = {
      useConfigJSON: [
      ...
      server: [
        ...
        },
        {
          ...
          type: 'input',
          message:
            'Git repository URL to be used for the authoring tool source code',
        ...
        },
        {
          ...
          type: 'input',
          message:
            'Git repository URL to be used for the framework source code',
      ...
      database: {
        dbConfig: [
          ...
          },
          {
            ...
            type: 'confirm',
            message:
              'Will you be using a full database connection URI? (all connection options in the URI)',
            default: false,
          },
        ],
        configureUri: [
          ...
          confirm: [
            {
              ...
              type: 'confirm',
              message:
                'Will you be using an SMTP server? (used for sending emails)',
              default: false,
            },
          ],
          ...
          configureService: [
            {
              ...
              type: 'input',
              message:
                'Which SMTP service (if any) will be used? (see https://github.com/andris9/nodemailer-wellknown#supported-services for a list of supported services.)',
              default: 'none',
            },
            {
              name: 'smtpUsername',
          ...
          configureConnectionUrl: [
            {
              ...
              type: 'input',
              message:
                'Custom connection URL: smtps://user%40gmail.com:pass@smtp.gmail.com/?pool=true',
              default: 'none',
            },
          ],
        },
    ...
    installHelpers.getInput(
      inputData.useConfigJSON,
      configOverrides,
      function (result) {
        console.log('');
...
function generatePromptOverrides() {
  ...
  const sessionSecret =
    (USE_CONFIG && configData.sessionSecret) ||
...
function configureDatabase(callback) {
  installHelpers.getInput(
    inputData.database.dbConfig,
    configOverrides,
    function (result) {
      addConfig(result);

      var isStandard = !installHelpers.inputHelpers.toBoolean(
        result.useConnectionUri
      );
      var config =
        inputData.database[isStandard ? 'configureStandard' : 'configureUri'];
...
function configureFeatures(callback) {
  async.series(
    [
      function smtp(cb) {
        installHelpers.getInput(
          inputData.features.smtp.confirm,
          configOverrides,
          function (result) {
            addConfig(result);
            ...
            installHelpers.getInput(
              inputData.features.smtp.confirmConnectionUrl,
              configOverrides,
              function (result) {
                addConfig(result);
                var smtpConfig;
                if (
                  installHelpers.inputHelpers.toBoolean(
                    result.useSmtpConnectionUrl
                  )
                ) {
                  smtpConfig = inputData.features.smtp.configure.concat(
                    inputData.features.smtp.configureConnectionUrl
                  );
                } else {
                  smtpConfig = inputData.features.smtp.configure.concat(
                    inputData.features.smtp.configureService
                  );
                }
                for (var i = 0, count = smtpConfig.length; i < count; i++) {
                  if (smtpConfig[i].name === 'rootUrl') {
                    smtpConfig[
                      i
                    ].default = `http://${configResults.serverName}:${configResults.serverPort}`;
                  }
                }
                installHelpers.getInput(
                  smtpConfig,
...
function configureMasterTenant(callback) {
  var onError = function (error) {
    ...
    return exit(
      1,
      'Failed to configure master tenant. Please check the console output.'
    );
  };
  if (!IS_INTERACTIVE || USE_CONFIG) {
  ...
  app.on('serverStarted', function () {
    if (USE_CONFIG && configOverrides.masterTenantName) {
       ...
       */
      inputData.tenant = inputData.tenant.filter(
        (item) => item.name !== 'masterTenantDisplayName'
      );
    }
    installHelpers.getInput(
      inputData.tenant,
      ...
      function (result) {
        ...
        app.tenantmanager.retrieveTenant(
          { name: result.masterTenantName },
          function (error, tenant) {
            ...
            if (!IS_INTERACTIVE) {
              return exit(
                1,
                `Tenant '${tenant.name}' already exists, automatic install cannot continue.`
              );
            }
            ...
            console.log(
              chalk.yellow(
                `Tenant '${tenant.name}' already exists. ${chalk.underline(
                  'It must be deleted for install to continue.'
                )}`
              )
            );
            installHelpers.getInput(
              inputData.tenantDelete,
              configOverrides,
              function (result) {
                console.log('');
                if (!result.confirm) {
...
function createSuperUser(callback) {
  var onError = function (error) {
    handleError(
      error,
      1,
      'Failed to create admin user account. Please check the console output.'
    );
  };
  console.log(
  ...
  fs.ensureDir('conf', function (error) {
    ...
    fs.writeJson(
      ...
      function (error) {
        if (error) {
          return handleError(
            `Failed to write configuration file to ${chalk.underline(
              'conf/config.json'
            )}.\n${error}`,
            1,
            'Install Failed.'
