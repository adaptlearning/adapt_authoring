...
Origin.prototype.startServer = function () {
  ...
  async.series([
    ...
    function(callback) {
      // Check the framework is up to date
      request({
        headers: {
          'User-Agent' : 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/41.0.2272.118 Safari/537.36'
        },
    ...
    function(callback) {
      if (installedBuilderVersion == latestBuilderTag) {
        ...
        console.log(chalk.bgYellow('You are currently running Adapt Builder %s - %s is now available'), installedBuilderVersion, latestBuilderTag);
      }

      if (installedFrameworkVersion == latestFrameworkTag) {
        console.log(chalk.green('Adapt Framework %s'), installedFrameworkVersion);
      } else {
        console.log(chalk.bgYellow('The Adapt Framework being used is %s - %s is now available'), installedFrameworkVersion, latestFrameworkTag);
      }
