...
Origin.prototype.startServer = function (options) {
  ...
  checkForUpdates(options, function(error) {
    if(error) {
      logger.log('error', `Failed to update install, ${error}`);
    }
    // configure server
    var serverOptions = {};
    if (!app.configuration || !app.configuration.getConfig('dbName')) {
...
function checkForUpdates(options, callback) {
  // TODO shouldn't be checking for this here...
