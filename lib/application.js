...
Origin.prototype.startServer = function (options) {
  ...
  var _checkPrerequisites = function(callback) {
    async.series([
      function __checkNodeVersion(cb) {
        installHelpers.checkNodeVersion(function(error) {
          if(error) return cb(new Error(`Node version check failed, ${error}`));
          cb();
        });
      },
      function __checkForupdates(cb) {
        if(configuration.getConfig('isTestEnvironment') || options.skipVersionCheck) {
          return cb();
        }
        checkForUpdates(function(error) {
          if(error) cb(new Error(`Check for updates failed, ${error}`));
          cb();
        });
      }
    ], callback);
  };
  _checkPrerequisites(function(err, result) {
    if(err) {
