...
var DEFAULT_SERVER_PORT = 3000,
  TEMP_DIR = path.join(require('./configuration').serverRoot, 'temp'),
  FRAMEWORK_DIR = 'adapt_framework',
...
Origin.prototype.startServer = function () {
  ...
  app.createServer(serverOptions, function (error, server) {
    ...
    function installFrameworkComponents() {
      var command = exec('adapt install', {cwd: path.join(require('./configuration').serverRoot, FRAMEWORK_DIR) },
        function(error, stdout, stderr) {
          if (stdout.length != 0) { 
            ...
            app.sockets.emit('installed:framework');
          }
