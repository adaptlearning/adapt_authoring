...
Origin.prototype.startServer = function () {
  ...
  if (process.version !== 'v0.10.33') {
    logger.log('warn', "You are running an unsupported version of NodeJS (" + process.version + "). This application has only been fully tested on NodeJS v0.10.33. It is recommended that you use that version instead.");
  }
...
Origin.prototype.run = function () {
  ...
  this.once('modulesReady', function (app) {
    ...
    try {
      ...
      logger.log('error', 'Error in Origin.prototype.run()!  Did you remember to run \'grunt build\'?');
      logger.log('error', e); 
    }
  });
