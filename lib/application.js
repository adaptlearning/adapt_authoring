...
AdaptBuilder.prototype.preload = function(){
  ...
  self.on('moduleLoaded',function(){
    ...
    if(false === modError && false === modWait){
      console.log('Preload has completed');
...
AdaptBuilder.prototype.start = function() {
  ...
  this.once('modulesReady', function (app) {
    ...
    console.log('Server Started Listening on ' + app.configuration.getConfig('serverPort'));
 ...
 */
AdaptBuilder.prototype.ModulePreloader.defOpts = {events:{
                                                            preload:false,
                                                            moduleLoaded:false,
