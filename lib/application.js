...
Origin.prototype.preloadModule = function (name, module) {
  ...
  if (module.preload && 'function' === typeof module.preload) {
    ...
    module
      .preload(this)
      .on('preloadChange', this.modulePreloadEventHandler.bind(this));
  }
};
...
Origin.prototype.createServer = function (options, cb) {
  ...
  if (options.minimal) {
    ...
    server.use(
      express.static(
        path.join(require('./configuration').serverRoot, 'frontend', 'src')
      )
    );
  ...
  require('./database').getDatabase(function (err, db) {
    ...
    // TEMP setup of cors TODO: configure cors
    server.use(cors());
    ...
    server.use(
      express.static(
        path.join(require('./configuration').serverRoot, 'frontend', 'build')
      )
    );
    server.use(
      express.static(
        path.join(
          require('./configuration').serverRoot,
          'frontend',
          'src',
          'libraries'
        )
    ...
    if (!app.configuration.getConfig('isProduction')) {
      server.use(
        express.static(
          path.join(app.configuration.serverRoot, 'frontend', 'src')
        )
      );
    }
  ...
  checkPrerequisites(options, function (err, result) {
    ...
    app.createServer(serverOptions, function (error, server) {
      ...
      httpServer.on('error', (error) => {
        ...
        logger.log(
          'error',
          `Port ${error.port} already in use. Please stop the server before continuing.`
        );
        process.exit();
      });
      app._httpServer = httpServer.listen(port, function () {
        ...
        app.emit(
          serverOptions.minimal ? 'minimalServerStarted' : 'serverStarted',
          app.server
        );
        ...
        var writeRebuildFile = function (courseFolder, callback) {
          ...
          var buildFolder = path.join(
            courseFolder,
            OutputConstants.Folders.Build
          );
          ...
          fs.exists(buildFolder, function (exists) {
            ...
            logger.log(
              'info',
              'Writing build to ' +
                path.join(buildFolder, OutputConstants.Filenames.Rebuild)
            );
            fs.writeFile(
              path.join(buildFolder, OutputConstants.Filenames.Rebuild),
              '',
        ...
        app.on('rebuildCourse', function (tenantId, courseId) {
          logger.log(
            'info',
            'Event:rebuildCourse triggered for Tenant: ' +
              tenantId +
              ' Course: ' +
              courseId
          );
...
Origin.prototype.run = function (options) {
  ...
  this.once('modulesReady', function (app) {
    try {
      ...
      var phraseStrings = JSON.parse(
        fs.readFileSync(
          path.join(configuration.serverRoot, 'routes', 'lang', 'en.json'),
          { encoding: 'utf8' }
        )
      );
...
function checkPrerequisites(options, callback) {
  async.series(
    [
      ...
      function (cb) {
        if (
          configuration.getConfig('isTestEnvironment') ||
          options.skipVersionCheck
        ) {
          return cb();
        }
        checkForUpdates((error) =>
          cb(error ? new Error(`Check for updates failed, ${error}`) : null)
        );
      },
    ],
    callback
    ...
    installHelpers.getUpdateData(function (error, updateData) {
      ...
      if (updateData && updateData.adapt_authoring) {
        logger.log(
          'info',
          chalk.yellow(
            `${app.polyglot.t('app.productname')} v${
              installedData.adapt_authoring
            } (${updateData.adapt_authoring} is now available).`
          )
        );
      } else {
        logger.log(
          'info',
          chalk.green(
            `${app.polyglot.t('app.productname')} ${
              installedData.adapt_authoring
            }.`
          )
        );
      ...
      if (updateData && updateData.adapt_framework) {
        logger.log(
          'info',
          chalk.yellow(
            `Adapt framework v${installedData.adapt_framework} (${updateData.adapt_framework} is now available).`
          )
        );
      } else {
        logger.log(
          'info',
          chalk.green(`Adapt framework ${installedData.adapt_framework}.`)
        );
      }
      if (updateData) {
        logger.log(
          'info',
          `Run ${chalk.bgRed('node upgrade.js')} to update your install.`
        );
      }
      callback();
    });
