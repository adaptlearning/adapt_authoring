    ...
    app.createServer(serverOptions, function (error, server) {
      ...
      app._httpServer = httpServer.listen(port, function() {
        ...
        app.on("rebuildCourse", function(tenantId, courseId) {
          logger.log('info', 'Event:rebuildCourse triggered for Tenant: ' + tenantId.toString() + ' Course: ' + courseId);

          var OutputConstants = require('./outputmanager').Constants;
          var courseFolder = path.join(configuration.tempDir, configuration.getConfig('masterTenantID'), OutputConstants.Folders.Framework, OutputConstants.Folders.AllCourses, tenantId.toString(), courseId);
