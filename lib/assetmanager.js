...
exports = module.exports = {
    ...
    return {
      ...
      moduleLoaded : function (modloaded) {
        ...
        if(modloaded === WAITFOR){
          ...
          rest.post('/asset/buildthumbs', instance.buildThumbnails.bind(instance));
          ...
          rest.post('/asset/syncworkspaces', instance.syncWorkspaces.bind(instance));
  ...
  buildThumbnails: function(req, res, next) {
    var self = this;
    var user = usermanager.getCurrentUser();
    this.hasPermission('create', user._id, user.tenant._id, {}, function (err, isAllowed) {
      if(!isAllowed) return next(new Error("Sorry, you don't have permission to create asset thumbnails"));
      // we want to check all assets for thumbnails
      self.retrieveAsset({}, { jsonOnly:true }, function(error, assets) {
        ...
        async.each(assets, function(asset, done) {
          filestorage.getStorage(asset.repository, function(error, storage) {
            if(error) return next(error);
            storage.getFileStats(storage.resolvePath(asset.thumbnailPath), function(error, stats) {
              if(error && error.code !== 'ENOENT') return next(error);
              else if(!error) return done();
              // thumb either doesn't exist, or is broken
              storage.createThumbnail(storage.resolvePath(asset.path), asset.assetType, thumbOpts, function(error, thumbPath) {
                if(error) return next(error);
                self.updateAsset({ _id: asset._id }, { thumbnailPath: thumbPath }, done);
              });
            });
          });
        }, function(error) {
  ...
  syncWorkspaces: function(req, res, next) {
    logger.log('info', 'assetmanager.syncWorkspaces');
    var app = require('./application')();
    var self = this;
    var user = usermanager.getCurrentUser();
    this.hasPermission('update', user._id, user.tenant._id, {}, function (err, isAllowed) {
      if(!isAllowed) return next(new Error("Sorry, you don't have permission to update assets"));
      // get all assets
      self.retrieveAsset({}, { jsonOnly:true }, function(error, assets) {
        if(error) return next(error);
        async.each(assets, function(asset, done) {
          // check for courseassets linked to current asset
          app.contentmanager.getContentPlugin('courseasset', function(error, plugin) {
            if(error) return done(error);
            plugin.retrieve({ _assetId: asset._id }, function(error, courseassets) {
              if(error) return done(error);
              if(courseassets.length > 0) {
                logger.log('info', 'Starting to generate workspaces for ' + asset.title);
                var workspaces = {
                  component: [],
                  block: [],
                  article: [],
                  page: [],
                  menu: [],
                  // make sure we keep courses
                  course: asset.workspaces.course || []
                };
                // build up workspaces with parents of courseasset._contentType
                async.each(courseassets, function(courseasset, done2) {
                  // store what we already know
                  storeWorkspace(courseasset._contentType, courseasset._contentTypeId, workspaces);
                  var contentTypes = Object.keys(workspaces);
                  var i = _.indexOf(contentTypes, courseasset._contentType);
                  if(i === -1) {
                    return done2("Unknown content type " + courseasset._contentType);
                  }
                  contentTypes = contentTypes.slice(i+1);
                  var nextId = courseasset._contentTypeParentId;
                  // go through the remaining content types (starting at the right point - i - in the hierarchy)
                  async.eachSeries(contentTypes, function(contentType, done3) {
                    var contentPluginType = (contentType === 'menu' || contentType === 'page') ? 'contentobject' : contentType;
                    app.contentmanager.getContentPlugin(contentPluginType, function(error, plugin) {
                      if(error) return done3(error);
                      plugin.retrieve({ _id: nextId }, function(error, contentObjects) {
                        if(error) return done3(error);
                        var co = contentObjects[0];
                        if(!co) { // this may be ok, not every content object has a menu
                          if(contentType !== 'menu') { // not ok
                            return done3('No ' + contentType + ' (' + contentPluginType + ') found with ID ' + nextId + ' (' + asset.title + ')');
                          }
                          return done3();
                        }
                        // update workspaces
                        storeWorkspace(contentType, co._id.toString(), workspaces);
                        nextId = co._parentId;
                        done3();
                      });
                    });
                  ...
                  if(error) return done(error);
                  logger.log('info', 'Workspaces generated for ' + asset.title);
                  self.updateAsset({ _id: asset._id }, { workspaces: workspaces }, done);
                });
              } else {
                logger.log('info', 'Skipping ' + asset.title + ' (not used in any courses)');
                done();
              }
            })
          });
    ...
    /*
    * Utility function to make sure the workspaces object is a set
    */
    function storeWorkspace(contentType, id, workspaces) {
      if(_.indexOf(workspaces[contentType],id) < 0) {
        workspaces[contentType].push(id);
      }
    }
  },
  ...
  postAsset: function (req, res, next) {
    ...
    form.parse(req, function (error, fields, files) {
      ...
      filestorage.getStorage(repository, function (error, storage) {
        ...
        self.generateFileLocationData(file, function(error, fileData) {
          if (error) {
            return next(error);
          }
          var fileOptions = {
    ...
    form.parse(req, function (error, fields, files) {
      ...
      self.retrieveAsset({ _id: req.params.id }, function (error, assetRecs) {
        ...
        filestorage.getStorage(assetRec.repository, function (error, storage) {
          ...
          // use query, can't depend on form.parse->fields
          var delta = {
            title: req.query.title,
            description: req.query.description,
            workspaces: (req.query.workspaces) ? JSON.parse(req.query.workspaces) : {},
            tags: req.query.tags && req.query.tags.split(',') || [],
          };
          var file = files.file;
          // only overwrite the file if a new one is provided
          if(file) {
            self.generateFileLocationData(file, function(error, fileData) {
              if(error) {
                return next(error);
              ...
              storage.getFileStats(fileData.path, function(error, stats) {
                if(error && error.code !== 'ENOENT') {
                  ...
                  return res.status(403).json({
                    success: false,
                    message: 'Cannot upload file, it already exists!'
                  });
                }
                ...
                storage.processFileUpload(file, fileData.path, fileOptions, function (error, storedFile) {
                  ...
                  async.parallel([
                    function deleteImage(cb) {
                      if(assetRec.path === storedFile.path) {
                        return cb();
                      }
                      storage.deleteFile(assetRec.path, function(error) {
                        // TODO only want to catch ENOENT
                        cb();
                      });
                    },
                    function deleteThumb(cb) {
                      if(assetRec.thumbnailPath === storedFile.thumbnailPath) {
                        return cb();
                      }
                      storage.deleteFile(assetRec.thumbnailPath, function(error) {
                        // TODO only want to catch ENOENT
                        cb();
                      });
                    }
                    ...
                    // update doc with delta + storedFile
                    self.updateAsset({ _id: assetRec._id }, _.extend(delta, storedFile), function (error, assetRec) {
                      if (error) {
                        return next(error);
                      }
  ...
  generateFileLocationData: function(file, callback) {
    var hash = crypto.createHash('sha1');
    fs.createReadStream(file.path, { flags: 'r', autoClose: true })
      .on('data', function(data) {
        hash.update(data, 'utf8');
      })
      .on('close', function() {
        var filehash = hash.digest('hex');
        var dir = path.join('assets', filehash.substr(0,2), filehash.substr(2,2));
        var ext = path.extname(file.name);
        var name = filehash + ext;
        callback(null, {
          name: name,
          ext: ext,
          dir: dir,
