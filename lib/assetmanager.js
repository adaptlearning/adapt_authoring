...
exports = module.exports = {
  ...
  preload: function (app) {
    var preloader = new app.ModulePreloader(app, MODNAME, {
      events: this.preloadHandle(app, this),
    });
    return preloader;
  },
    ...
    return {
      ...
      moduleLoaded: function (modloaded) {
        ...
        if (modloaded === WAITFOR) {
          ...
          rest.put(
            '/asset/restore/:id',
            instance.restoreAssetByID.bind(instance)
          );
          ...
          rest.get(
            '/shared/asset/:id',
            instance.serveSharedAsset.bind(instance)
          );
      ...
      db.create('asset', data, function (err, doc) {
        ...
        permissions.createPolicy(user._id, function (err, policy) {
          if (err) {
            logger.log(
              'error',
              'there was an error granting editing permissions',
              err
            );
          }
              ...
              filestorage.getStorage(
                ...
                function (error, storage) {
                  if (error) {
                    ...
                    logger.log(
                      'error',
                      'Failed to retrieve filestorage repository',
                      assetRec.repository
                    );
                    return next(error);
                  }
                    ...
                    return storage.deleteFile(
                      ...
                      function (delErr) {
                        if (delErr)
                          logger.log(
                            'error',
                            'Failed to delete stored file in assetmanager',
                            storedFile.path
                          );
                        _respondError(
                          'An error occured during upload, please contact an Administrator.'
                        );
                      }
                    );
                  }
        ...
        function (error, assetRecs) {
          ...
          filestorage.getStorage(
            ...
            function (error, storage) {
              ...
              storage.createReadStream(
                assetRec.path,
                { bufferSize: STREAM_BUFFER_SIZE, forceMaster: true },
                function (stream) {
                  res.writeHead(200, {
                    'Content-Type': assetRec.mimeType,
                    'Content-Length': assetRec.size,
