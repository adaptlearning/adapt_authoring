    ...
    form.parse(req, function (error, fields, files) {
      ...
      self.retrieveAsset({ _id: req.params.id }, function (error, assetRecs) {
        ...
        filestorage.getStorage(assetRec.repository, function (error, storage) {
          ...
          // TODO can't always depend on form.parse->fields ???
          var formData = Object.keys(fields).length > 0 ? fields : req.query;
          var delta = {
            title: formData.title,
            description: formData.description,
            workspaces: (formData.workspaces) ? JSON.parse(formData.workspaces) : {},
          ...
          if(file) {
            fileData = self.generateFileLocationData(file, assetRec);
