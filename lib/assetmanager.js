...
exports = module.exports = {
  ...
  postAsset: function (req, res, next) {
    ...
    form.parse(req, function (error, fields, files) {
      ...
      filestorage.getStorage(fields.repository, function (error, storage) {
        if (error) {
          return next(error);
        }
        ...
        rs.on('end', function () {
          ...
          var filepath = path.join(directory, filehash);
          ...
          storage.processFileUpload(file, filepath, function (error, storedFile) {
            ...
            var path = storedFile.path;

            // This next line is required in case the environment is Windows
            path = path.replace(' ', '\ ');
            ...
            probe(path, function(err, probeData) {
              ...
              switch (storedFile.type.split('/')[0]) {
                case 'image':
                asset.metadata = {width: probeData.streams[0].width, height: probeData.streams[0].height};
                break;

                case 'video':
                asset.metadata = {duration: probeData.streams[0].duration, width: probeData.streams[0].width, height: probeData.streams[0].height}
                break;

                case 'audio':
                asset.metadata = {duration: probeData.streams[0].duration};
                break;
