        ...
        async.each(assets, function(asset, done) {
          if(!asset.workspaces) {
            app.contentmanager.getContentPlugin('courseasset', function(error, plugin) {
              ...
              plugin.retrieve({ _assetId: asset._id }, function(error, results) {
                if(error) return next(error);
                ...
                if(results.length > 0) {
                  ...
                  async.each(results, function(courseasset, done2) {
                    // store what we know
                    workspaces[courseasset._contentType].push(courseasset._contentTypeId);
                    // go through the remaining content types
                    var contentTypes = Object.keys(workspaces);
                    var i = _.indexOf(contentTypes, courseasset._contentType);
                    if(i > -1) contentTypes = contentTypes.slice(i+1);
                    var nextId = courseasset._contentTypeParentId;

                    async.eachSeries(contentTypes, function(contentType, done3) {
                      if(!nextId) return done3(); // something's gone wrong

                      app.contentmanager.getContentPlugin(contentType, function(error, plugin) {
                        if(error) return next(error);

                        plugin.retrieve({ _id: nextId }, function(error, results) {
                          if(error) return next(error);

                          var result = results[0];
                          workspaces[contentType] && workspaces[contentType].push(result._id.toString());
                          nextId = result._parentId || false;
