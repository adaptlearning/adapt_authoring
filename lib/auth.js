...
AuthPlugin.prototype.authenticate = function (req, res, next) {
  throw new Error(
    'AuthPlugin#authenticate must be overridden in extending object'
  );
};
...
AuthPlugin.prototype.internalRegisterUser = function (
  retypePasswordRequired,
  user,
  cb
) {
  throw new Error(
    'AuthPlugin#internalRegisterUser must be overridden in extending object'
  );
};

AuthPlugin.prototype.resetPassword = function (req, res, next) {
  throw new Error(
    'AuthPlugin#resetPassword must be overridden in extending object'
  );
};

AuthPlugin.prototype.internalUpdatePassword = function (user, cb) {
  throw new Error(
    'AuthPlugin#internalResetPassword must be overridden in extending object'
  );
};

AuthPlugin.prototype.generateResetToken = function (req, res, next) {
  throw new Error(
    'AuthPlugin#generateResetToken must be overridden in extending object'
  );
};

AuthPlugin.prototype.internalGenerateResetToken = function (user, cb) {
  throw new Error(
    'AuthPlugin#internalGenerateResetToken must be overridden in extending object'
  );
};
...
exports = module.exports = {
  ...
  getAvailableAuthPluginsSync: function () {
    return fs
      .readdirSync(path.join(configuration.serverRoot, 'plugins', 'auth'))
      .filter(junk.not);
  },
    ...
    moduleLoaded: function (modloaded) {
      ...
      if (modloaded === WAITFOR) {
        ...
        rest.post('/login', function (req, res, next) {
          instance.getAuthPlugin(
            configuration.getConfig('auth'),
            function (error, authPlugin) {
              if (error) {
        ...
        rest.get('/authcheck', function (req, res, next) {
          ...
          permissions.getUserPermissions(
            user._id,
            function (error, userPermissions) {
              if (error) {
                return next(error);
              }
