            ...
            app.contentmanager.getContentPlugin(pluginType, function(err, plugin) {
              ...
              installHelpers.getLatestFrameworkVersion(function(error, frameworkVersion) {
                ...
                if (!semver.satisfies(semver.clean(frameworkVersion), packageInfo[pluginName].pkgMeta.framework)) {
                  var error = `Unable to install ${packageInfo[pluginName].pkgMeta.name} (${packageInfo[pluginName].framework}) as it is not supported in the current version of of the Adapt framework (${frameworkVersion})`;
