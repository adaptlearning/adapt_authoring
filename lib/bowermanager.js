...
BowerManager.prototype.installPlugin = function(pluginName, pluginVersion, callback) {
  ...
  fs.readFile(path.join(configuration.getConfig('root'), 'version.json'), 'utf8', function(err, version) {
...
BowerManager.prototype.importPackage = function (plugin, packageInfo, options, callback) {
  ...
  fs.exists(schemaPath, function (exists) {
    ...
    fs.readFile(schemaPath, function (err, data) {
      ...
      var destination = path.join(configuration.tempDir, configuration.getConfig('masterTenantID'), 'adapt_framework', 'src', plugin.bowerConfig.srcLocation, pkgMeta.name);
