 ...
 */
BowerManager.prototype.installPlugin = function(pluginName, pluginVersion, options, callback) {
  // Shuffle params.
          ...
          .on('end', function (packageInfo) {
            ...
            app.contentmanager.getContentPlugin(pluginType, function(err, plugin) {
              ...
              installHelpers.getLatestFrameworkVersion(function(error, frameworkVersion) {
                ...
                var importOptions = _.extend({}, bowerOptions, options);

                if (!packageInfo[pluginName].pkgMeta.framework) {
                  return self.importPackage(plugin, packageInfo[pluginName], importOptions, callback);
                }
                ...
                self.importPackage(plugin, packageInfo[pluginName], importOptions, callback);
              });
            });
          });
      });
...
* @param {function} callback - Callback function
 */
BowerManager.prototype.installLatestCompatibleVersion = function (pluginName, options, callback) {
  // Shuffle params.
  if ('function' === typeof options) {
    callback = options;
    options = {};
        ...
        .on('end', function (latestInfo) {
          ...
          installHelpers.getInstalledFrameworkVersion(function(error, installedFrameworkVersion) {
            ...
            async.doUntil(function iterator(cb) {
              ...
              self.installPlugin(pluginName, latestInfo.versions[index], options, callback);
            });
          });
        });
    });
...
BowerManager.prototype.importPackage = function (plugin, packageInfo, options, callback) {
  ...
  fs.exists(schemaPath, function (exists) {
    ...
    fs.readFile(schemaPath, function (err, data) {
      ...
      var frameworkDest = options.frameworkDest || path.join(app.configuration.getConfig('root').toString(), 'temp', app.configuration.getConfig('masterTenantID').toString(), 'adapt_framework');
      var destination = path.join(frameworkDest, 'src', plugin.bowerConfig.srcLocation, pkgMeta.name);
