...
BowerManager.prototype.installPlugin = function(pluginName, pluginVersion, callback) {
  ...
  // Interrogate the version.json file on installation as we cannot rely on including it via a 'require' call.
  fs.readFile(path.join(app.configuration.getConfig('root'), 'version.json'), 'utf8', function(err, version) {
    ...
    rimraf(path.join(bowerOptions.directory, pluginName), function (err) {
      ...
      bower.commands.search(packageName, bowerOptions)
        .on('error', callback)
        ...
        var bowerPackage = _.findWhere(results, {name: packageName});

        // Install the plugin from bower.
        bower.commands.install([bowerPackage.name], { save: true }, bowerOptions)
          ...
          .on('end', function (packageInfo) { 
            ...
            app.contentmanager.getContentPlugin(pluginType, function(err, plugin) {
              ...
              if (packageInfo[pluginName].pkgMeta.framework) {
                ...
                if (semver.satisfies(semver.clean(version.adapt_framework), packageInfo[pluginName].pkgMeta.framework)) {
                  self.importPackage(plugin, packageInfo[pluginName], bowerOptions, callback);
                } else {
                  logger.log('error', 'Unable to install ' + packageInfo[pluginName].pkgMeta.name + '(' + packageInfo[pluginName].framework + ') as it is not supported in the current version of of the Adapt framework (' +  version.adapt_framework + ')');
                  return callback('Unable to install ' + packageInfo[pluginName].pkgMeta.name + ' as it is not supported in the current version of of the Adapt framework');
                }
              } else {
                self.importPackage(plugin, packageInfo[pluginName], bowerOptions, callback);   
              }
...
BowerManager.prototype.importPackage = function (plugin, packageInfo, options, callback) {
  ...
  fs.exists(schemaPath, function (exists) {
    ...
    fs.readFile(schemaPath, function (err, data) {
      ...
      // Build a path to the destination working folder.
      var destination = path.join(app.configuration.getConfig('root').toString(), 'temp', app.configuration.getConfig('masterTenantID').toString(), 'adapt_framework', 'src', plugin.bowerConfig.srcLocation, pkgMeta.name);
