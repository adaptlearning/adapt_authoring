...
BowerManager.prototype.importPackage = function (plugin, packageInfo, options, callback) {
  ...
  fs.exists(schemaPath, function (exists) {
    ...
    fs.readFile(schemaPath, function (err, data) {
      ...
      var destination = options.dest || path.join(app.configuration.getConfig('root').toString(), 'temp', app.configuration.getConfig('masterTenantID').toString(), 'adapt_framework', 'src', plugin.bowerConfig.srcLocation, pkgMeta.name);
