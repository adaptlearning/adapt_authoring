...
BowerManager.prototype.extractPackageInfo = function (plugin, pkgMeta, schema) {
  ...
  plugin.extra &&
    plugin.extra.forEach(function (key) {
      if (pkgMeta[key]) {
 ...
 */
BowerManager.prototype.installPlugin = function (
  pluginName,
  pluginVersion,
  callback
) {
  ...
  var packageName =
    pluginVersion == '*' ? pluginName : pluginName + '#' + pluginVersion;
  ...
  fs.remove(path.join(bowerOptions.directory, pluginName), function (err) {
    ...
    bower.commands
      .search(pluginName, bowerOptions)
      .on('error', callback)
      .on('end', function (results) {
        ...
        bower.commands
          .install([packageName], { save: true }, bowerOptions)
          ...
          .on('end', function (packageInfo) {
            ...
            var pluginType = packageInfo[pluginName].pkgMeta.hasOwnProperty(
              'component'
            )
              ? 'component'
            ...
            if (pluginType == '') {
              logger.log(
                'error',
                'Unable to identify pluginType for ' + packageName
              );
              return callback(
                'Unable to identify pluginType for ' + packageName
              );
            ...
            app.contentmanager.getContentPlugin(
              ...
              function (err, plugin) {
                ...
                installHelpers.getInstalledFrameworkVersion(function (
                  error,
                  frameworkVersion
                ) {
                  if (error) {
                  ...
                  if (
                    !semver.satisfies(
                      semver.clean(frameworkVersion),
                      packageInfo[pluginName].pkgMeta.framework
                    )
                  ) {
 ...
 * @param {function} callback - Callback function
 */
BowerManager.prototype.installLatestCompatibleVersion = function (
  pluginName,
  callback
) {
  var self = this;
          ...
          installHelpers.getInstalledFrameworkVersion(function (
            ...
            async.doUntil(
              function iterator(cb) {
                bower.commands
                  .info(bowerPackage.url + '#' + latestInfo.versions[++index])
                  .on('error', cb)
                    ...
                    app.db.retrieve(
                      ...
                      (error, results) => {
                        ...
                        if (
                          ...
                          semver.satisfies(
                            installedFrameworkVersion,
                            installedPlugin.framework
                          )
                        ) {
                          return callback(
                            'Skipping as no newer compatible version found'
                          );
                        }
 ...
 */
BowerManager.prototype.importPackage = function (
  plugin,
  packageInfo,
  options,
  callback
  ...
  var schemaPath = path.join(
    packageInfo.canonicalDir,
    options._adaptSchemaFile
  );
  ...
  fs.exists(schemaPath, function (exists) {
    ...
    fs.readFile(schemaPath, function (err, data) {
      ...
      var destination = path.join(
        ...
        'temp',
        app.configuration.getConfig('masterTenantID'),
        'adapt_framework',
        'src',
      ...
      fs.remove(destination, function (err) {
        ...
        fs.mkdirs(destination, function (err) {
          ...
          fs.copy(packageInfo.canonicalDir, destination, function (err) {
            ...
            logger.log(
              'info',
              'Successfully copied ' + pkgMeta.name + ' to ' + destination
            );
              ...
              function (err, results) {
                ...
                if (results && results.length !== 0) {
                  ...
                  if (installedPlugin.version === package.version) {
                    ...
                    if (options.strict) {
                      return callback(
                        "Can't add " + pluginString + ': verion already exists'
                      );
                    }
                  ...
                  keysToPersist.forEach(
                    (key) => (package[key] = installedPlugin[key])
                  );
                }
                ...
                db.create(
                  ...
                  function (err, newPlugin) {
                    if (err) {
                      ...
                      logger.log(
                        'error',
                        'Failed to add package: ' + pluginString,
                        err
                      );
                      return callback(null);
                    }
                    ...
                    db.retrieve(
                      ...
                      function (err, results) {
                        if (err) {
                          ...
                          logger.log(
                            'error',
                            'Failed to retrieve previous packages: ' +
                              err.message,
                            err
                          );
                        }
                        ...
                        db.destroy(
                          ...
                          function (err) {
                            ...
                            logger.log(
                              'info',
                              'Successfully removed versions of ' +
                                package.name +
                                ' (' +
                                plugin.getPluginType() +
                                ') older than ' +
...
exports = module.exports = {
  ...
  preload: function (app) {
    var preloader = new app.ModulePreloader(app, MODNAME, {
      events: this.preloadHandle(app, new BowerManager()),
    });
    return preloader;
  },
