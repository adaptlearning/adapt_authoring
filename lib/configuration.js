...
Configuration.prototype.load = function (filePath, callback) {
  ...
  fs.exists(filePath, function(exists){
    if (exists) {
      ...
      return callback(new MissingConfigError());
    }
  });
};
 ...
 *
 * @param {String} [name] - the configuration to get, or an empty string to get the entire config object
 * @return {value} - the value of the config if set, or an empty string
 * @api public
 */
Configuration.prototype.getConfig = function(name, all) {;
  if (name && typeof name === 'string') {
