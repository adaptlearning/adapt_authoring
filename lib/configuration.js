  ...
  fs.readFile(filePath, function (err, data) {
    if (err) {
      ...
      try {
        ...
        logger.log(
          'error',
          'Invalid Configuration JSON :: ' + filePath + '\n' + e.toString()
        );
        return callback(e);
      }
      return callback();
...
Configuration.prototype.preload = function (app) {
  var preloader = new app.ModulePreloader(app, MODNAME, {
    events: { preload: preloadHandle(app, this) },
  });
  return preloader;
};
...
function preloadHandle(app, instance) {
  return function () {
    ...
    instance.load(
      path.join(instance.serverRoot, app.options.configFile),
      function (err) {
        if (err) {
