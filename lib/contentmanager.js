...
ContentPlugin.prototype.create = function(data, next) {
  console.log('ContentPlugin.create');
...
ContentManager.prototype.loadContentPlugins = function(next) {
  var self = this;
  pluginmanager.getManager().getPlugin('content', function(err, plugins) {
    async.eachSeries(Object.keys(plugins), function(pluginName, nextPlugin) {
              ...
              function(callback) {
                ...
                async.eachSeries(Object.keys(parentRelationship), function(contentType, cb) {
                  ...
                  async.eachSeries(clipboard[contentType], function(item, cb2) {
                    console.log('parentCollection:', contentType, parentRelationship[contentType], typeToCollection[parentRelationship[contentType]]);
                    ...
                    console.log(' ', item.title, item._extensions);

                    self.create(collectionToType[contentType], item, function(error, newItem) {
                      if (error) return cb2(error);

                      console.log('    new item: ' + newItem.title, newItem._extensions);
