    ...
    try {
      ...
      db.addModel(modelName, schema, function (err, fullSchema) {
        ...
        if (err) {
          ...
          app.contentmanager.addContentSchema(modelName, schema);
        }
          ...
          async.series([
            ...
            function(callback) {
               async.eachSeries(newestResults, function (schema, processCallback) {
                ...
                switch (plugin.type) {
                  case 'extension':
                    async.each(Object.keys(schema.properties.pluginLocations.properties), function(key, nestedCallback) {
                      var pluginLocations = schema.properties.pluginLocations.properties[key];
              ...
              function(callback) {
                ...
                if (clipboard['contentObjects'].length !== 0) {
                  async.eachSeries(clipboard['contentObjects'], function(item, cb) {
                    ...
                    var newParentId = typeof(map['contentObjects'][item._parentId.toString()]) !== 'undefined' && map['contentObjects'][item._parentId.toString()] != null
                                      ? map['contentObjects'][item._parentId.toString()]
                                      : parentObject._id;
              ...
              function(callback) {
                ...
                if (clipboard['blocks'].length !== 0) {
                  async.eachSeries(clipboard['blocks'], function(item, cb) {
                    var previousId = item._id.toString();
                    var newParentId = typeof(map['articles'][item._parentId.toString()]) !== 'undefined'
                                      ? map['articles'][item._parentId.toString()]
                                      : parentObject._id;
              ...
              function(callback) {
                ...
                if (clipboard['components'].length !== 0) {
                  async.eachSeries(clipboard['components'], function(item, cb) {
                    var previousId = item._id.toString();
                    var newParentId = typeof(map['blocks'][item._parentId.toString()]) !== 'undefined'
                                      ? map['blocks'][item._parentId.toString()]
                                      : parentObject._id;
