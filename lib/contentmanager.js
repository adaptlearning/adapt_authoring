 ...
 */
ContentManager.prototype.addModel = function (name, schema, callback) {
  var self = this;
  database.getDatabase(function(err, db){
    if (!name || 'string' !== typeof name) {
      ...
      var jsonImporter = db.getImporter('json-schema', 'http://localhost/contentSchema/' + name + '.json');

      jsonImporter.importSchema(schema, function () {

        name = name.toLowerCase();

        jsonImporter.exportORM(function (orm) {
          // let the database keep track of models
          var mdl = db.addModel(name, orm);
...
ContentManager.prototype.setupRoutes = function () {
  ...
  rest.get('/content/:type', function (req, res, next) {
    ...
    var qry = req.query; //@todo direct pass to retrieve() should be avoided
    that.retrieve(type, qry, function (error, results) {
      if (error) {
        res.statusCode = error instanceof ContentTypeError ? 400 : 500;
  ...
  fs.readdir(schemaDirectory, function (error, results) {
    if (error) {
      ...
      for (; i < results.length; ++i) {
        ...
        //@todo swap the following for Async equivilent solution
        if(fs.statSync(fullPath).isDirectory()){
          var schemaPath = path.join(fullPath, path.basename(fullPath) + '.json');
          var modelName = path.basename(schemaPath, '.json');
          if(fs.existsSync(schemaPath)){
            var schema = fs.readFileSync(schemaPath);
            schema = JSON.parse(schema);

            waitList.push(modelName);

            instance.addModel(modelName, schema, (function (mname) {
              var _mname = mname;
              return function () {
                var idx = waitList.indexOf(mname);
                if(idx > -1){
            ...
            logger.log('error', 'Content schema directory missing the json file : ' + fullPath);
          }
        }
      }
    }
  ...
  return {
      ...
      moduleLoaded : function(modloaded){
        ...
        if(modloaded === WAITFOR){
          ...
          var schemaDirectory = path.join(configuration.serverRoot, 'lib', 'dml', 'schema','contentSchema');
          instance.loadSchemas(schemaDirectory, function() {
