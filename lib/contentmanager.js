...
ContentPlugin.prototype.getModelName = function () {
  logger.log(
    'error',
    'ContentPlugin#getModelName must be implemented by extending objects!'
  );
    ...
    try {
      ...
      db.addModel(modelName, schema, function (err, fullSchema) {
        ...
        if (err) {
          logger.log(
            'error',
            'Error importing content schema: ' + err.message,
            err
          );
        } else {
      ...
      logger.log(
        'error',
        'failed to parse schema file at ' + schemaPath,
        error
      );
      return next(error);
    }
 ...
 */
ContentPlugin.prototype.hasPermission = function (
  action,
  userId,
  tenantId,
  contentItem,
 ...
 * @param {callback} next
 */
ContentPlugin.prototype.iterateSiblings = function (
  parentId,
  options,
  iterator,
  next
 ...
 * @param {callback} next
 */
ContentPlugin.prototype.destroyChildren = function (
  parentId,
  parentAttribute,
  next
) {
...
ContentManager.prototype.addContentHook = function (
  action,
  targetContent,
  options,
  processor
...
ContentManager.prototype.processContentHooks = function (
  action,
  contentType,
  data,
  options,
  ...
  var hooks =
    this._contentHooks[contentType] &&
  ...
  async.map(
    hooks,
    function processHooks(hook, next) {
      hook(data, next);
    },
...
ContentManager.prototype.getContentPlugin = function (type, cb) {
  ...
  pluginManager.getPlugin('content', type, function (error, pluginInfo) {
    if (error) {
      return cb(
        new ContentTypeError('content type plugin ' + type + ' was not found')
      );
    }
...
ContentManager.prototype.setupRoutes = function () {
  ...
  rest.get('/content/schema', function (req, res, next) {
    ...
    function generateComponentProperties(schema) {
      ...
      data.properties = helpers.cloneObject(
        that._contentSchemas.component.properties
      );
    ...
    function processPluginLocations(pPluginType, schema, memo, callback) {
      ...
      try {
        var schemaPluginLocations =
          schema.properties.pluginLocations.properties;
      } catch (e) {
  ...
  rest.post('/content/:type', function (req, res, next) {
    ...
    if (!val || 'object' !== typeof val) {
      ...
      return res.json({
        success: false,
        message: 'request body was not a valid object',
      });
    }
  ...
  rest.put('/content/:type/switch/:id/:switchid', function (req, res, next) {
    ...
    if (
      ...
      (!switchVal || 'object' !== typeof switchVal)
    ) {
      res.statusCode = 400;
      return res.json({
        success: false,
      ...
      async.series(
        [
          ...
          function (callback) {
            ...
            db.retrieve(
              'article',
              { _parentId: contentObjects[0]._id },
              function (error, results) {
                if (error) {
                  return callback(error);
                }
        ...
        function (err) {
          if (err) {
            ...
            return res
              .status(500)
              .json({ success: false, message: 'Error adding clipboard data' });
          }
          res
            .status(200)
            .json({ success: true, message: 'ok', clipboardId: clipboardId });
        }
      );
    });
  });
  ...
  rest.post('/content/clipboard/paste', function (req, res, next) {
    ...
    database.getDatabase(function (error, db) {
      ...
      db.retrieve(
        'clipboard',
        { id: id, createdBy: user._id },
        function (error, results) {
          if (error) {
            return res
              .status(500)
              .json({ success: false, message: 'Clipboard data not found' });
          }

          if (results.length !== 1) {
            return res.status(500).json({
              success: false,
              message: 'More than one clipboard found',
            });
          }
          ...
          async.series(
            [
              function (callback) {
                ...
                db.retrieve(
                  parentRelationship[clipboard.referenceType],
                  { _id: parentId },
                  function (error, results) {
                    if (error) {
                      return callback(error);
                    ...
                    if (
                      clipboard.referenceType === keyMap.ContentObject &&
                      clipboard[
                        keyMap.ContentObject
                      ][0]._courseId.toString() ===
                        clipboard._courseId.toString()
                    ) {
              ...
              function (callback) {
                if (
                  clipboard[clipboard.referenceType].length === 1 &&
                  sortOrder
                ) {
              ...
              function (callback) {
                ...
                async.eachSeries(
                  clipboard[keyMap.ContentObject],
                  function (item, cb) {
              ...
              function (callback) {
                ...
                async.eachSeries(
                  clipboard[keyMap.Article],
                  function (item, cb) {
                    var previousId = item._id.toString();
                    var previousParentId = item._parentId.toString();
                    var newParentId =
                      typeof map[keyMap.ContentObject][previousParentId] !==
                        'undefined' &&
                      map[keyMap.ContentObject][previousParentId] != null
              ...
              function (callback) {
                ...
                async.eachSeries(
                  clipboard[keyMap.Block],
                  function (item, cb) {
                    var previousId = item._id.toString();
                    var previousParentId = item._parentId.toString();
                    var newParentId =
                      typeof map[keyMap.Article][previousParentId] !==
                      'undefined'
              ...
              function (callback) {
                ...
                async.eachSeries(
                  clipboard[keyMap.Component],
                  function (item, cb) {
                    var previousId = item._id.toString();
                    var previousParentId = item._parentId.toString();
                    var newParentId =
                      typeof map[keyMap.Block][previousParentId] !== 'undefined'
            ...
            function (err) {
              if (err) {
                ...
                return res.status(500).json({
                  success: false,
                  message: 'Error pasting clipboard data',
                });
              }
              ...
              db.destroy('clipboard', { id: id }, function (error) {
                var newId = _.values(
                  map[
                    _.find(Object.keys(map), function (key) {
                      return !_.isEmpty(map[key]);
                    })
                  ]
                )[0];
      ...
      function (key, cb) {
        self.getContentPlugin(key, function (error, contentPlugin) {
          if (error) {
            logger.log(
              'error',
              'Failed to load content plugin of type: ' + key,
              error
            );
            return cb(error);
          }
 ...
 * 3. Call `post` hooks
 */
['create', 'retrieve', 'update', 'destroy'].forEach(function (
  el,
  index,
  array
) {
...
exports = module.exports = {
  ...
  preload: function (app) {
    var preloader = new app.ModulePreloader(app, MODNAME, {
      events: this.preloadHandle(app, new ContentManager()),
    });
    return preloader;
  },
      ...
      moduleLoaded: function (modloaded) {
        ...
        if (modloaded === WAITFOR) {
          ...
          instance.loadContentPlugins(function () {
            database.addDatabaseHook(
              'create',
              instance.onDatabaseCreated.bind(instance)
            );
            preloader.emit(
              'preloadChange',
              MODNAME,
