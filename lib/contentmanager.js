    ...
    async.eachSeries(
      ...
      function (childType, cb) {
        app.contentmanager.getContentPlugin(childType, function (error, plugin) {
          ...
          plugin.destroy({_parentId:parentId}, cb);
        });
      },
      function (error) {
        if (error) {
...
ContentManager.prototype.addContentHook = function (action, targetContent, processor) {
  ...
  this._contentHooks[targetContent][action].push(processor);
};
...
ContentManager.prototype.processContentHooks = function (action, contentType, data, cb) {
