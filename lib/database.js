    ...
    dbInstance.loadSchemas(path.join(getSchemaPath(), 'system'), function (error) {
      processDatabaseHooks('create', dbInstance, function(err) {
        if (err) {
          ...
          delete _databases[type][tenant._id];
          return next(err);
        }
        dbInstance.readyState = ReadyStates.READY;
        _databases[type][tenant._id] = dbInstance;
...
function getDatabase(next, tenantId, type) {
  ...
  if(tenantId === config.dbName || tenantId === config.masterTenantID) {
    return _handleMasterDB(type, config.masterTenantID, next) ;
  }
  _handleSlaveDB(type, tenantId, next);
}

function _handleMasterDB(type, tenantId, next) {
  const config = configuration.getConfig();
  const dbInstance = _databases[type][tenantId];

  if (dbInstance) { // db instance already exists
    if(dbInstance.readyState === ReadyStates.READY) { // ...and is connected
      return next(null, dbInstance);
    }
    // db instance is still connecting, so wait until this is done
    return app.on('create:masterdatabase', dbInstance => next(null, dbInstance));
  }
  const tenant = { _id: tenantId };
  tenant.database = { dbName, dbHost, dbUser, dbPass, dbPort } = config;
...
function _handleSlaveDB(type, tenantId, next) {
  ...
  app.tenantmanager.retrieveTenant({ _id: tenantId }, function (err, tenant) {
    ...
    createDatabaseInstance(type, tenant, next);
  });
}
...
function checkConnection(cb) {
  ...
  var connectionURI = configuration.getConfig('dbConnectionUri');
  var haveConnectionConfig = connectionURI || (host && port) ? true : false;

  if(!name || !haveConnectionConfig) {
