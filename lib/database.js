 ...
 */
function getDatabase(cback, tenantId, type) {
  if (!this.dbs){
  ...
  var databaseObject = false;
  if (!tenantId) {
    var user = app.usermanager.getCurrentUser();
    if (user.tenant) {
      tenantId = user.tenant._id;
    } else {
      return cback(new Error("Failed to determine user's tenant!"));
    }
  }

  databaseObject = app.tenantmanager.getDatabaseConfig(tenantId);

  if (this.dbs[type] && this.dbs[type][tenantId]){
    cback(null, this.dbs[type][tenantId]);
  } else {
    if (!this.dbs[type]) {
      this.dbs[type]= Object.create(null);
    }
    ...
    fs.exists(dbPath, function (exists) {
      if (exists) {
        ...
        this.dbs[type][tenantId] = dbInstance;
        dbInstance.connect(databaseObject);
        dbInstance.loadSchemas(path.join(getSchemaPath(), 'system'), function (error) {
          processDatabaseHooks('create', dbInstance, function (err) {
            ...
            // keep the event emission for now
            if (tenantId == configuration.getConfig('masterTenantID')) {
              app.emit('create:masterdatabase', dbInstance);
            } else {
              app.emit('create:database', dbInstance);
            }

            return cback(null, dbInstance);
          });
        }.bind(this));
      } else {
        this.dbs[type][tenantId] = new Database();
        cback(new errors.notthere(dbPath), this.dbs[type][tenantId]);
      }
    }.bind(this));
  }
}
