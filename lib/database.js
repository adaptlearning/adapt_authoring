...
function getDatabase(cback, dbname, type){
  ...
  if ('string' !== typeof dbname) {
    dbname = (process.domain && process.domain.auth && process.domain.auth.currentTenant)?process.domain.auth.currentTenant:configuration.getConfig('dbName');
  }
...
function preload(app, done) {
  //might be better to use process.nextTick to defer this?
