 ...
 */
Database.prototype.getImporter = function (type) {
  logger.log('warn', 'Database#getImporter must be implemented by extending object!');
  throw new Error('Database#getImporter not implemented');
};
...
function getDatabase(cback, dbname, type){
  ...
  if (this.dbs[type] && this.dbs[type][dbname]){
    ...
    var dbPath = path.join(configuration.getConfig('root'), 'lib', 'dml', type, type + '.js');
