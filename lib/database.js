...
const errors = {
  ...
  nocallback: function (fnName) {
    ...
    this.message =
      'Database function was called without being provided a callback :: ' +
      fnName;
  },
};
...
Database.prototype.connect = function () {
  logger.log(
    'warn',
    'Database.connect must be implemented by extending object!'
  );
...
Database.prototype.disconnect = function (next) {
  logger.log(
    'warn',
    'Database.disconnect must be implemented by extending object!'
  );
  throw new Error(
    'Database.disconnect must be implemented by extending object!'
  );
...
Database.prototype.isStale = function (tenant) {
  logger.log(
    'warn',
    'Database.isStale must be implemented by extending object!'
  );
...
Database.prototype.isValidIdentifier = function (id) {
  logger.log(
    'warn',
    'Database.isValidIdentifier must be implemented by extending object!'
  );
  throw new Error(
    'Database.isValidIdentifier must be implemented by extending object!'
  );
...
Database.prototype.loadSchemas = function (schemaDirectory, callback) {
  logger.log(
    'warn',
    'Database.loadSchemas must be implemented by extending object!'
  );
  throw new Error(
    'Database.loadSchemas must be implemented by extending object!'
  );
...
Database.prototype.addSchema = function (modelName, schema) {
  logger.log(
    'warn',
    'Database#addSchema must be implemented by extending object!'
  );
  throw new Error(
    'Database#addSchema must be implemented by extending object!'
  );
...
Database.prototype.getModel = function (modelName) {
  logger.log(
    'warn',
    'Database#getModel must be implemented by extending object!'
  );
...
Database.prototype.getModelNames = function () {
  logger.log(
    'warn',
    'Database#getModelNames must be implemented by extending object!'
  );
  throw new Error(
    'Database#getModelNames must be implemented by extending object!'
  );
...
Database.prototype.buildPopulator = function (options) {
  logger.log(
    'warn',
    'Database#buildPopulator must be implemented by extending object!'
  );
  throw new Error(
    'Database#buildPopulator must be implemented by extending object!'
  );
...
Database.prototype.buildQuery = function (options) {
  logger.log(
    'warn',
    'Database#buildQuery must be implemented by extending object!'
  );
  throw new Error(
    'Database#buildQuery must be implemented by extending object!'
  );
...
Database.prototype.create = function (objectType, objectData, callback) {
  logger.log(
    'warn',
    'Database#create must be implemented by extending object!'
  );
...
Database.prototype.retrieve = function (objectType, search, options, callback) {
  logger.log(
    'warn',
    'Database#retrieve must be implemented by extending object!'
  );
 ...
 * @param {function} callback - of the form function (error, results) ...
 */
Database.prototype.retrieveOne = function (
  objectType,
  search,
  options,
  callback
...
Database.prototype.update = function (
  ...
  logger.log(
    'warn',
    'Database#update must be implemented by extending object!'
  );
...
Database.prototype.destroy = function (objectType, conditions, callback) {
  logger.log(
    'warn',
    'Database#destroy must be implemented by extending object!'
  );
...
Database.prototype.importSchema = function (uri, schema, next) {
  logger.log(
    'warn',
    'Database#importSchema must be implemented by extending object!'
  );
...
Database.prototype.exportResults = function (results, next) {
  logger.log(
    'warn',
    'Database#exportResults must be implemented by extending object!'
  );
...
function resolveSchemaPath(uri) {
  ...
  if (uri.substr(0, local.length) === local) {
    ...
    if ('string' === typeof pathMap[pathType]) {
      return path.join(
        pathMap[pathType],
        localPath.substr(pathType.length).replace(/\//g, path.sep)
      );
    }
  }
...
function createDatabaseInstance(type, dbConfig, next) {
  ...
  dbInstance.connect(dbConfig).then(function () {
    ...
    dbInstance.loadSchemas(
      path.join(getSchemaPath(), 'system'),
      function (error) {
...
function _handleMasterDB(type, next) {
  ...
  createDatabaseInstance(
    type,
    { dbName, dbHost, dbUser, dbPass, dbPort },
    (err, db) => {
      if (err) {
...
function checkConnection(cb) {
  ...
  const haveConnectionConfig =
    dbConnectionUri || (dbHost && dbPort) ? true : false;

  if (!dbName || !haveConnectionConfig) {
    return cb(
      'Cannot check database connection, missing settings in config.json.'
    );
  }
  getDatabase(function (error, db) {
    ...
    if (db.conn.readyState !== 1) {
      return cb(
        `Cannot connect to the MongoDB '${dbName}' at '${dbHost}:${dbPort}'. Please check the details are correct and the database is running.`
      );
    }
    cb();
  }, dbName);
