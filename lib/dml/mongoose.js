        ...
        fs.readFile(fullPath, function (error, data) {
          if (error) {
            ...
            try {
              ...
              // try and "require" the schema
              try {
                schema = require(fullPath);
                this.addModel(modelName, schema);
              } catch (requireError) {
                logger.log('error', 'failed to require schema file at ' + fullPath, requireError);
              }
            }
          }
...
MongooseDB.prototype.buildPopulator = function (options) {
  ...
  if (Object.prototype.toString.apply(options) === '[object Array]') {
    ...
    _.keys(options).forEach(function (key) {
      ...
      populator[key] = {path: key, select: null};
      if (Object.prototype.toString.apply(options[key]) === '[object Array]') {
        populator[key].select = options[key].join(' ');
      } else if ('string' === typeof options[key]) {
        populator[key].select = options[key];
      }
    });
  }
 ...
 * @param {function} callback - of the form function (error, results) ...
 */
MongooseDB.prototype.retrieve = function(objectType, options, callback) {
  var search = options.search || {};
  var operators = options.operators || null;
  var populator = options.populator || null;
  var fields = options.fields || null;
  var Model = false;

  if (Model = this.getModel(objectType)) {
    var query = Model.find(search, fields, operators);
    if (populator && 'string' === typeof populator) {
