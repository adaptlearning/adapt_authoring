...
function cloneFramework(app, next) {
  ...
  var command = exec('git clone ' + GIT_FRAMEWORK_CLONE_URL, {cwd: require('./configuration').serverRoot},
      function (error, stdout, stderr) {
        if (error) {
          app && app.sockets.emit('output', {type: 'error', value: error.toString()});
          logger.log('error', 'exec error: ' + error);

          return next({type: 'error', message: error.toString()}); 
        }

        if (stderr.length != 0) {
          app && app.sockets.emit('output', {type: 'error', value: stderr});
          logger.log('error', 'stderr: ' + stderr);
         
          return next({type: 'error', message: stderr});          
        }
        ...
        if (stdout.length != 0) { 
          ...
          if (stdout.indexOf('done') > -1) {
            ...
            return next({type: 'error', message: 'Unexpected error in cloneFramework'});       
          }
        } else {
          return next({type: 'error', message: 'Unexpected error in cloneFramework'});       
        }
  ...
  var comand = exec('npm install --production', {cwd: path.join(require('./configuration').serverRoot, FRAMEWORK_DIR) },
    function(error, stdout, stderr) {
      ...
      if (stdout.length != 0) { 
        ...
        return next({type: 'error', message: 'Unexpected error in installFramework'});     
      }
  });
};
  ...
  var command = exec('adapt install', {cwd: path.join(require('./configuration').serverRoot, FRAMEWORK_DIR) },
    function(error, stdout, stderr) {
      if (error) {
        ...
        return next({type: 'error', value: error.toString()}); 
      }
      ...
      if (stderr.length != 0) {
        ...
        return next({type: 'error', value: stderr});
      }
      ...
      if (stdout.length != 0) { 
       ...
       return next({type: 'error', value: 'Unexpected error in installFrameworkComponents'});    
      }
  });
};
