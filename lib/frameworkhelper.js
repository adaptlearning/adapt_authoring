...
function cloneFramework(next, frameworkRepository, frameworkRevision) {
  fs.exists(path.join(serverRoot, FRAMEWORK_DIR), function (exists) {
    if (exists) {
      ...
      console.log(
        'The Adapt Framework directory was found, skipping installation.'
      );
      return next(null);
    }

    console.log(
      'The Adapt Framework was not found. It will now be installed...'
    );
...
function installFramework(next, frameworkRevision) {
  ...
  child.on('exit', function (error, stdout, stderr) {
    ...
    fs.remove(
      path.join(serverRoot, FRAMEWORK_DIR, 'src', 'course'),
      function (err) {
        if (err) {
          console.log('ERROR:', error);
        }
