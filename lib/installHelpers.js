...
function installDependencies(opts, callback) {
  ...
  console.log("rqewrqwerqwnjrkewnrlqwerqwerwerwerwerw", opts)

  fs.remove(path.join(cwd, 'node_modules'), function (error) {
    if (error) {
      return callback(error);
    }
    console.log("this is where deps are installed", cwd, "23423412341234")
    execCommand('npm run preinstall && npm install --loglevel error --production', { cwd: cwd }, function (error) {
      hideSpinner();
      if (error) {
        console.log("is here an ERROR here" + error)
        return callback(error);
      }
