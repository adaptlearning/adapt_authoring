...
var DEFAULT_THEME_REPO = `https://github.com/${DEFAULT_GITHUB_ORG}/adapt-theme-cms.git`;
...
function cloneTheme(opts, callback) {
  ...
  const themeDirectory = `"${opts.directory}\\src\\theme\\adapt-contrib-vanilla"`;
  checkOptions(opts, "clone", function (error) {
    if (error) {
      return callback(error);
    }
    ...
    fs.remove(
      `${opts.directory}\\src\\theme\\adapt-contrib-vanilla`,
      function (error) {
        if (error) {
          hideSpinner();
          return callback(error);
