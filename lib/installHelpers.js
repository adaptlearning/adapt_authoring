...
function fetchRepo(opts, callback) {
  ...
  var _doFetch = function() {
    execCommand(`git fetch ${REMOTE_NAME}`, execOptions, callback);
  };
  checkOptions(opts, 'fetch', function(error) {
    if(error) {
      return callback(error);
    }
    execCommand(`git remote set-url ${REMOTE_NAME} ${opts.repository}`, execOptions, function(error) {
      if(!error) {
        return _doFetch();
      }
      // unexpected error, so try adding the remote in case it doesn't exist yet
      execCommand(`git remote add ${REMOTE_NAME} ${opts.repository}`, execOptions, function(error) {
        if(error) return callback(error);
        _doFetch();
      });
    });
  ...
  fetchRepo(opts, function(error) {
    ...
    checkOptions(opts, 'update', function(error) {
      ...
      execCommand(`git reset --hard && git checkout ${opts.revision}`, {
        ...
        hideSpinner();
        if (error) {
          return callback(`Failed to update ${shortDir} to '${opts.revision}'. Does the revision exist?`);
        }
...
function execCommand(cmd, opts, callback) {
  ...
  exec(cmd, _.extend({ stdio: [0, 'pipe', 'pipe'] }, opts)).on('exit', callback);
}
