...
function checkDependencies(checks, callback) {
  ...
  async.each(checks, function(check, cb) {
    ...
    if(hasErrored) {
      return callback(new Error('Some of the prerequisites could not be found, see above for details.'));
    }
    callback();
  });
}
...
function checkNodeVersion(callback) {
  ...
  if(semver.gt(installedVersion, semver.coerce(requiredVersion))) {
    return callback(null, `You are using Node.js v${installedVersion} which is not yet supported by Adapt. If you encounter issues, please downgrade to v${requiredVersion}.`);
  }
  if(!semver.satisfies(installedVersion, requiredVersion)) {
    return callback(new Error(`The application requires Node.js v${requiredVersion} to run, found v${installedVersion}. Please make sure you have a compatible version.`));
  }
  callback();
}

function checkGitInstalled(callback) {
  execCommand('git --version', {}, function(error, stdOut) {
    if (error) return callback('git could not be found, please check it is installed');
    callback();
  });
}

function checkGruntCliInstalled(callback) {
  execCommand('grunt --version', {}, function(error, stdOut) {
    if (error) return callback('grunt-cli could not be found, please check it is installed');
    callback();
  });
}
...
function checkGithubConnection(callback) {
  ...
  request({
    ...
    if (error) return callback(`Failed to connect to ${url}`);
    callback();
  });
}
...
function checkDbConnection(callback) {
  database.checkConnection(function(error) {
    if (error) return callback(`Couldn't connect to the database, please check the connection details are valid`);
    callback();
  });
}
...
function checkFfmpegInstalled(callback) {
  ...
  execCommand('ffmpeg -version', {}, function(error, stdOut) {
    if (error) return callback(null, 'Local FFmpeg could not be found, please check it is installed');
    callback();
  });
}
