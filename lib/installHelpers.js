...
function checkNodeVersion(callback) {
  ...
  if(!semver.satisfies(installedVersion, requiredVersion)) {
    return callback(`The application requires Node.js v${requiredVersion} to run, found v${installedVersion}. Please make sure you have a compatible version.`);
  }
  callback();
}
...
function checkSmtpConnection(callback) {
  ...
  (new mailer()).testConnection(error => callback(error && error.message || null));
}
...
function checkFfmpegInstalled(callback) {
  ...
  execCommand('ffmpeg -version', {}, error => {
    callback(error ? 'Local FFmpeg could not be found, please check it is installed' : null);
  });
}
