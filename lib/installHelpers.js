...
function checkGitInstalled(result, gitCb) {
  execCommand('git --version', {}, function(error, stdOut) {
    if (error) {
      result.errors.push(new Error('git must be installed'));
    }
    gitCb();
  });
}
...
function checkGruntCliInstalled(result, gruntCb) {
  execCommand('grunt --version', {}, function(error, stdOut) {
    if (error) {
      result.errors.push(new Error('grunt-cli must be installed'));
    }
    gruntCb();
  });
}
...
function checkGithubConnection(result, githubCb) {
  ...
  request({
    ...
    if (error) {
      result.errors.push(new Error(`Failed to´connect to ${url}`));
    }
    githubCb();
  });
}
