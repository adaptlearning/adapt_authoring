...
function checkNodeVersion(callback) {
  ...
  if(semver.gt(installedVersion, semver.coerce(requiredVersion))) {
    warn(`You are using Node.js v${installedVersion} which is not yet supported by Adapt. If you encounter issues, please downgrade to v${requiredVersion}.`);
    return callback();
  }
...
*/
function checkDependencies(checks, message, callback) {
  const result = [];
  async.applyEach(checks, result, function(error) {
...
function checkRequiredDependencies(callback) {
  checkDependencies([
    checkGitInstalled,
...
function checkOptionalDependencies(callback) {
  checkDependencies([
...
function checkGitInstalled(result, gitCb) {
...
function checkGruntCliInstalled(result, gruntCb) {
  execCommand('grunt --version', {}, function(error, stdOut) {
    if (error) {
      result.push('- grunt-cli');
    }
    gruntCb();
  });
}

function checkGithubConnection(result, githubCb) {
  ...
  request({
    ...
    if (error) {
      result.push(`- failed to connect to ${url}`);
    }
    githubCb();
  });
}
...
function checkLocalMongoDbConnection(result, callback) {
  execCommand('mongod --version', {}, function(error, stdOut) {
    if (error) {
      result.push('- local MongoDB');
    }
    callback();
  })
}

function checkFfmpegInstalled(result, callback) {
