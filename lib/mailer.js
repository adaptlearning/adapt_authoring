...
const validateSettings = function(settings) {
  ...
  if (settings.useSmtpConnectionUrl === true) {
    if (settings.smtpConnectionUrl === '') errors.push('smtpConnectionUrl');
  } else {
    if (settings.smtpService === '') errors.push('smtpService');
    if (settings.smtpUsername === '') errors.push('smtpUsername');
    if (settings.smtpPassword === '') errors.push('smtpPassword');
  }
  // generic configuration options
  if (settings.fromAddress === '') errors.push('fromAddress');
...
const generateFriendlyError = function(error) {
  ...
  switch(error.errno) {
    case 'ENOTFOUND':
      ...
      advice = `make sure you've specified the correct host`;
      break;
    case 'ECONNREFUSED':
    ...
    case 'ETIMEDOUT':
      ...
      advice = `check you've specified a valid port`;
      break;
    case 'EAUTH':
    ...
    default:
      ...
      console.log(error.errno, error.message);
  }
  return new Error(`Failed to verify SMTP connection. ${message} (${error.errno}), ${advice}`);
};
