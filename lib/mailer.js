...
const validateSettings = function (settings) {
  const errors = [
    settings.useSmtpConnectionUrl &&
      !settings.smtpConnectionUrl &&
      'smtpConnectionUrl',
  ...
  if (errors.length) {
    throw new Error(
      `Mailer requires the following settings to function: ${errors.toString()}`
    );
  }
};
...
const configure = function (options, callback) {
  if (!helpers.isValidEmail(options.to)) {
    return callback(
      new Error(`Can't send email to ${options.to}, not a valid email address`)
    );
  }
  if (!helpers.isValidEmail(options.from)) {
    return callback(
      new Error(
        `Can't send email from ${options.fromAddress}, not a valid email address`
      )
    );
  }
  ...
  template
    .render(options.templateData.name, options)
...
const generateFriendlyError = function (error) {
  ...
  return new Error(
    `Failed to verify SMTP connection. ${message} (${code}), ${advice}`
  );
};
