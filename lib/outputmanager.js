...
OutputPlugin.prototype.getCourseJSON = function (tenantId, courseId, next) {
  ...
  database.getDatabase(function (err, db) {
    ...
    async.reduce(
      Object.keys(Constants.CourseCollections),
      {},
      function (memo, collectionType, callback) {
...
function getConfigJson(courseId, callback) {
  origin().contentmanager.getContentPlugin(
    'config',
    function (err, contentPlugin) {
      if (err) {
        return callback(err);
      }
      contentPlugin.retrieve(
        { _courseId: courseId },
        {},
        function (err, config) {
          if (err) {
          ...
          if (config.length !== 1) {
            return callback(
              new Error('Preview/Publish: Unable to retrieve config.json')
            );
          }
          ...
          database.getDatabase(function (err, db) {
            ...
            db.retrieve(
              'component',
              { _courseId: courseId },
              { operators: { distinct: '_component' } },
              function (err, components) {
                db.retrieve(
                  'componenttype',
                  {},
                  function (err, componentTypes) {
                    if (err) {
                      return next(err);
                    }
                    ...
                    async.map(
                      ...
                      function (component, callback) {
                        var componentType = _.findWhere(componentTypes, {
                          component: component,
                        });
                      ...
                      function (err, uniqueComponentList) {
                        ...
                        callback(null, {
                          config: [flattenNestedObjects(configModel)],
                        });
                      }
                    );
                  }
...
function flattenNestedObjects(targetItem) {
  ['_extensions', 'menuSettings', 'themeSettings'].forEach(function (
    attribute
  ) {
...
OutputPlugin.prototype.sanitizeCourseJSON = function (mode, json, next) {
  ...
  try {
    ...
    async.waterfall(
      [
        function (callback) {
          self.generateIncludesForConfig(
            configJson,
            function (error, includes) {
              if (error) {
                return callback(error);
              }
    ...
    [
      ...
      (json, callback) => {
        this.generateIncludesForConfig(json.config[0], (error, includes) => {
          ...
          if (!includes) {
            return callback(
              new Error(`No plugins included in course ${courseId}`)
            );
          }
...
OutputPlugin.prototype.generateIncludesForConfig = function (config, callback) {
  ...
  async.eachSeries(
    ...
    function (plugin, next) {
      ...
      self.addPluginWithDependencies(
        pluginNames,
        source_root,
        plugin[0],
        plugin[1],
    ...
    function (error) {
       ...
       */
      if (
        _.indexOf(pluginNames, 'adapt-contrib-gmcq') > -1 &&
        _.indexOf(pluginNames, 'adapt-contrib-mcq') === -1
      ) {
...
function generateIncludedPlugins(config) {
  ...
  const pluginTypeInfos = [
    ...
    if (typeof val === 'string')
      return includedPlugins.push([pluginTypeInfo.folder, val]);
 ...
 * @param next callback
 */
OutputPlugin.prototype.addPluginWithDependencies = function (
  includedPlugins,
  source_root,
  plugin_folder,
  plugin_name,
  ...
  var self = this;
  var bower_file = path.join(
    source_root,
    plugin_folder,
    plugin_name,
    Constants.Filenames.Bower
  );
...
OutputPlugin.prototype.getUpdatedIncludePlugins = function (
  includedPlugins,
  plugin_name
) {
...
OutputPlugin.prototype.writeCourseJSON = function (
  jsonObject,
  destinationFolder,
  next
) {
  try {
    ...
    async.each(
      Object.keys(Constants.CourseCollections),
      function (collectionType, callback) {
        var propertiesToOmit =
          Constants.CourseCollections[collectionType].omitProps;

        if (propertiesToOmit) {
          // Remove any non-essential properties from the JSON.
          outputJson[collectionType] = _.omit(
        ...
        var filename =
          collectionType === 'config'
            ? path.join(
                destinationFolder,
                Constants.CourseCollections[collectionType].filename
              )
            : path.join(
                destinationFolder,
                outputJson['config']._defaultLanguage,
                Constants.CourseCollections[collectionType].filename
              );
...
OutputPlugin.prototype.applyTheme = function (
  tenantId,
  courseId,
  jsonObject,
  destinationFolder,
  ...
  database.getDatabase(function (err, db) {
    ...
    db.retrieve('themetype', { name: themeName }, {}, function (err, results) {
      ...
      var theme = results[0];
      var THEME_ROOT = path.join(
        configuration.tempDir,
        configuration.getConfig('masterTenantID'),
        Constants.Folders.Framework,
        Constants.Folders.Source,
        Constants.Folders.Theme
      );
      ...
      fs.remove(destinationFolder, function (err) {
        ...
        fs.copy(themeFolder, destinationFolder, function (err) {
          if (err) {
            ...
            return next(
              err,
              'Error copying ' + themeFolder + ' to ' + destinationFolder
            );
          }

          self.writeThemeVariables(
            courseId,
            theme,
            themeVariables,
            destinationFolder,
 ...
 * @param next
 */
OutputPlugin.prototype.writeThemeVariables = function (
  courseId,
  theme,
  themeVariables,
  destinationFolder,
  ...
  var destinationFile = path.join(
    destinationFolder,
    Constants.Folders.Less,
    Constants.Folders.CustomStyleDirectory,
    Constants.Filenames.Variables
  );
  var modifiedProperties = '';
  var props = {};
  var savedSettings = {};
  var SEPARATOR = '-';
  var themeAssetsFolder = path.join(
    destinationFolder,
    ...
    [
        ...
        async.eachSeries(
          ...
          function (key, innerCallback) {
            ...
            async.eachSeries(
              ...
              function (childKey, secondInnerCallback) {
                props[key + SEPARATOR + childKey] = childKey;
                theme.properties.variables[key + SEPARATOR + childKey] =
                  variables[key].properties[childKey];
          ...
          function (key, innerCallback) {
            if (key !== '_type') {
              if (typeof themeVariables[key] === 'object') {
                ...
                async.each(
                  Object.keys(themeVariables[key]),
                  function (childKey, secondInnerCallback) {
                    savedSettings[key + SEPARATOR + childKey] =
                      themeVariables[key][childKey];
                  ...
                  function (err) {
                    if (err) {
                      logger.log(
                        'error',
                        'Theme customisations 2 of 4 -- error flattening themeVariables'
                      );
                    }
                  }
                );
      ...
      function (seriesCallback) {
        ...
        async.each(
          ...
          function (prop, innerCallback) {
            ...
            if (
              savedSettings.hasOwnProperty(prop) &&
              themeProperty.default !== savedSettings[prop]
            ) {
              ...
              if (inputType.media === 'image' || inputType === 'Asset:image') {
                ...
                assetPathArray[assetPathArray.length - 1] = encodeURIComponent(
                  assetPathArray[assetPathArray.length - 1]
                );
                // Make the required substitution for image assets
                savedSettings[prop] =
                  '"' + assetPathArray.join('/').replace('course/', '') + '"';
              }

              modifiedProperties +=
                '@' + props[prop] + ': ' + savedSettings[prop] + ';\n';
            }
            innerCallback();
          },
          function (err) {
      ...
      function (seriesCallback) {
        ...
        database.getDatabase(function (err, db) {
          ...
          db.retrieve(
            'courseasset',
            { _courseId: courseId, _contentType: 'theme' },
...
OutputPlugin.prototype.writeCustomStyle = function (
  tenantId,
  courseId,
  destinationFolder,
  next
        ...
        if (results && results.length == 1) {
          if (results[0].customStyle) {
            ...
            var filename = path.join(
              ...
              Constants.Folders.Less,
              Constants.Folders.CustomStyleDirectory,
              Constants.Filenames.CustomStyle
            );
...
OutputPlugin.prototype.writeCourseAssets = function (
  tenantId,
  courseId,
                ...
                function (error, assets) {
                  ...
                  async.eachSeries(
                    ...
                    function (asset, callback) {
                      ...
                      Object.keys(Constants.CourseCollections).forEach(
                        function (key) {
                          jsonObject[key] = JSON.parse(
                            JSON.stringify(jsonObject[key]).replace(
                              replaceRegex,
                              newAssetPath
                            )
                          );
...
OutputPlugin.prototype.applyMenu = function (
  tenantId,
  courseId,
  jsonObject,
  destinationFolder,
    ...
    database.getDatabase(function (err, db) {
      ...
      db.retrieve('menutype', { name: menuName }, {}, function (err, results) {
        if (err) {
          ...
          if (results && results.length == 1) {
            var menu = results[0];
            var MENU_ROOT = path.join(
              configuration.tempDir,
              configuration.getConfig('masterTenantID'),
              Constants.Folders.Framework,
              Constants.Folders.Source,
              Constants.Folders.Menu
            );
...
OutputPlugin.prototype.preview = function (courseId, req, res, next) {
  logger.log(
    'error',
    'OutputPlugin#preview must be implemented by extending objects!'
  );
  throw new Error(
    'OutputPlugin#preview must be implemented by extending objects!'
  );
...
OutputPlugin.prototype.publish = function (courseId, req, res, next) {
  logger.log(
    'error',
    'OutputPlugin#publish must be implemented by extending objects!'
  );
  throw new Error(
    'OutputPlugin#publish must be implemented by extending objects!'
  );
...
OutputPlugin.prototype.export = function (courseId, req, res, next) {
  logger.log(
    'error',
    'OutputPlugin#export must be implemented by extending objects!'
  );
  throw new Error(
    'OutputPlugin#export must be implemented by extending objects!'
  );
...
exports = module.exports = {
  ...
  preload: function (app) {
    var preloader = new app.ModulePreloader(app, MODNAME, {
      events: this.preloadHandle(app, new OutputManager()),
    });
    return preloader;
  },
