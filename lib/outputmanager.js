            ...
            async.eachSeries(results, function(result, callback) {
              ...
              assetmanager.retrieveAsset({ _id: result._assetId }, function (error, assets) {
                ...
                if (!processedAssets[asset.filename]) {
                  ...
                  var objects = ['config', 'course', 'article', 'block', 'component'];
                  for(var i = 0, count = objects.length; i < count; i++) {
                    jsonObject[objects[i]] = JSON.parse(JSON.stringify(jsonObject[objects[i]]).replace(replaceRegex, newAssetPath));
                  }
                  ...
                  for (var i = 0; i < jsonObject['contentobject'].length; i++) {
                    ...
                    console.log('_graphic:', co.hasOwnProperty('_graphic'));
