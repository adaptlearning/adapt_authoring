...
OutputManager.prototype.setupRoutes = function () {
  ...
  // preview
  // NB: just a hint - not a hard implementation suggestion
  rest.get('/output/:type/preview/:courseid', function (req, res, next) {
    var type = req.params.type;
    var courseId = req.params.courseid;
    that.preview(type, courseId, req, res, function (error, result) {
      if (error) {
  ...
  rest.get('/output/:type/publish/:courseid', function (req, res, next) {
    var type = req.params.type;
    var courseId = req.params.courseid;
    that.publish(type, courseId, req, res, function (error, result) {
      if (error) {
        res.json({ success: false, message: error.message });
        return res.end();
      }
