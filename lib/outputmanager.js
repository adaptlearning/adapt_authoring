      ...
      masterDb.retrieve('themetype', {name: themeName}, {}, function(err, results) {
        ...
        fs.remove(destinationFolder, function(err) {
          ...
          fs.copy(themeFolder, destinationFolder, function (err) {
            if (err) {
              callback(err, 'Error copying ' + themeFolder + ' to ' + destinationFolder);
              return next(null, themeName);
            } 

            // Make subsitutions in variables.less for any customisations
            fs.readFile(path.join(destinationFolder, 'less', 'variables.less'), function(err, file) {
              if (err) {
                logger.log('error', err);
                return next(null, themeName);
              }
              ...
              if (theme.properties) {
                ...
                async.series([
                    ...
                    async.each(props, function(prop, innerCallback) {
                      ...
                      if (savedSettings.hasOwnProperty(prop) && theme.properties[prop].default !== savedSettings[prop]) {
                        ...
                        if (theme.properties[prop].inputType === 'Asset:image') {
                          ...
                          // Replace spaces
                          savedSettings[prop] = encodeURIComponent(savedSettings[prop]);
                        }                                  
                  ...
                  function(seriesCallback) {
                    ...
                    fs.writeFile(path.join(destinationFolder, 'less', 'variables.less'), file, 'utf8', function (err) {
                      if (err) {
                  ...
                  function(seriesCallback) {
                    var processedAssets = [],
                      processedTitles = [];
                      destinationFolder = path.join(destinationFolder, 'assets');
