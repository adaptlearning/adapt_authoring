...
OutputPlugin.prototype.generateIncludesForCourse = function(courseId, next) {
    ...
    database.getDatabase(function(error, db) {
      ...
      db.retrieve('config', { _courseId: courseId }, { jsonOnly: true }, function (error, results) {
            ...
            if(results.length !== 1) {
                return next(new Error('Unexpected results returned for course ' + courseId + ' (' + results.length + ')', self));
            }

            var includes = self.generateIncludesForConfig(results[0]);
            if(!includes) next(new Error("No plugins included for course " + courseId));
            else next(null, includes);
      });
    });
};
