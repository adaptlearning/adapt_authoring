...
exports = module.exports = {
  ...
  getUserPolicyStatements: function (userId, callback) {
    ...
    database.getDatabase(function(err, db) {
      // first retrieve bare policies
      db.retrieve('policy', {user: userId}, function (error, records) {
        if (err) {
          return callback(err);
        }

        records && records.forEach(function (p) {
          statements = statements.concat(p.statement);
        });

        // next, retrieve role assignments
        db.retrieve('user', { _id: userId }, { fields: 'roles _tenantId invalidatePermissionCache', populate: ['roles'] }, function (err, records) {
          if (err) {
            return cb(err);
          }
          ...
          roles.forEach(function (r) {
            r.statement.forEach(function (s, index) {
