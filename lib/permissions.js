...
function PolicyStatementParser(action, resource, statements, callback) {
  ...
  if (
    !(action && 'string' === typeof action) ||
    !(resource && 'string' === typeof resource) ||
    !(
      statements &&
      Object.prototype.toString.apply(statements) === '[object Array]'
    )
  ) {
    ...
    parse: function (cb) {
      ...
      for (let i = 0; i < statements.length; i++) {
        ...
        const sharesNamespace =
          statementToken.namespace === resourceToken.namespace;
        const matchesResource = this.captures(
          statementToken.identifier,
          resourceToken.identifier
        );
...
exports = module.exports = {
      ...
      function (err, db) {
        ...
        if (Object.prototype.toString.apply(actions) !== '[object Array]') {
          if ('string' !== typeof actions) {
            throw new Error(
              'actions parameter must be string or array',
              actions
            );
          }
  ...
  hasPermission: function (userId, action, resource, callback) {
    this.getUserPolicyStatements(userId, function (error, statements) {
      ...
      if (!statements || 0 === statements.length) {
        // no policies set for this user
        return callback(null, false);
      }
      new PolicyStatementParser(action, resource, statements, function (
        error,
        parser
      ) {
        if (error) {
  ...
  getUserPolicyStatements: function (userId, callback) {
    database.getDatabase(function (err, db) {
      db.retrieve(
        'user',
        { _id: userId },
        { fields: 'roles _tenantId', populate: ['roles'] },
          ...
          function () {
            ...
            async.eachSeries(
              ...
              function (userStatement, uniqueStatementsCallback) {
                var apiString = userStatement.resource
                  .replace('urn:x-adapt:', '')
                  .replace('/api', '');
