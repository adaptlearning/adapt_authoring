...
exports = module.exports = {
   ...
   *
   * @param {string} route - a route to ignore
   */
  ...
  shouldIgnore: function (route) {
    return (-1 !== ignoreRoutes.indexOf(route));
  },
  ...
  policyChecker: function () {
    ...
    return function (req, res, next) {
      ...
      if (!user) {
        res.writeHead(403, { 'Content-Type': 'application/json' }); // 403 forbidden ok?
        res.end();
        return;
      }
      ...
      var action = that.getActionType(req.method); // at this point, we can only enforce CRUD methods
      that.hasPermission(user.id, action, resource, function (error, allowed) {
        if (error) {
          // summat went wrong
