...
function PolicyStatementParser(action, resource, statements, callback) {
  ...
  callback(null, {
     ...
     */
    parse: function (cb) {
      var cachedPermissions = app.usermanager.getSessionVariable('cachedPermissions') || { allowed: [], disallowed: [] };

      if(cachedPermissions.allowed.includes(resource)) {
        return cb(null, true);
      }
      if(cachedPermissions.disallowed.includes(resource)) {
      ...
      for(var i = 0, statement = statements[i], count = statements.length; i < count; statement = statements[++i]) {
        if (!statement || !statement.action.includes(action)) {
          continue;
        }
        var toks = this.tokens(statement.resource);
        // must share a namespace and identifier must capture the supplied identifier
        if (!toks || toks.namespace !== resourceTok.namespace || !this.captures(toks.identifier, resourceTok.identifier)) {
          continue;
        }
        // we have a match, now we need to check the effect
        var effect = statement.effect.toLowerCase();
        if(effect === 'deny') { // explicit deny has precedence
          allowed = false;
          break;
        }
        if(effect === 'allow') { // explicitly require 'allow' (may be overridden by further statements)
          allowed = true;
        }
      }
      // save the result in the session
      allowed ? cachedPermissions.allowed.push(resource) : cachedPermissions.disallowed.push(resource);
...
exports = module.exports = {
  ...
  policyChecker: function () {
    ...
    return function (req, res, next) {
      ...
      var resource = that.buildResourceString(user.tenant._id, req.path);
