  ...
  database.getDatabase(function (err, db) {
    ...
    db.retrieve('plugin', null, function (error, results) {
      if (error || !results) {
        logger.log(
          'error',
          'Failed to retrieve Plugin records with error',
          error
        );
        callback(error);
      } else {
        ...
        for (var res = 0; res < results.length; ++res) {
          ...
          if (
            !plugin[plugin.type] ||
            'object' !== typeof plugins[plugin.type]
          ) {
  ...
  if (!this.plugins || this._markedDirty || forceReload) {
    ...
    for (var type = 0; type < pluginTypes.length; ++type) {
      ...
      if (
        ...
        typeof this.plugins[pluginType] !== 'object'
      ) {
      ...
      if (pluginDirs.length) {
        for (var dir = 0; dir < pluginDirs.length; ++dir) {
          ...
          try {
            ...
            logger.log(
              'error',
              'Failed to load plugin at path ' + pluginDir,
              error
            );
          }
        }
...
PluginManager.prototype.getPlugin = function (type, pluginName, callback) {
  ...
  if ('function' === typeof pluginName) {
    // callback was provided, pluginName wasn't
    callback = pluginName;
    pluginName = false;
  }
      ...
      db.retrieve('plugin', {}, function (error, results) {
        if (error) {
          ...
          for (var res = 0; res < results.length; ++res) {
            ...
            if (
              !installed[plugin.type] ||
              'object' !== typeof installed[plugin.type]
            ) {
          ...
          for (var type in diskPlugins) {
            for (var pluginName in diskPlugins[type]) {
              ...
              var diskVersion =
                diskPlugins[type][pluginName].version.split('.');
              var dbVersion = installed[type][pluginName].version.split('.');
              if (
                diskVersion.length != 3 ||
...
PluginManager.prototype.uninstallPlugin = function (pluginInfo, callback) {
  database.getDatabase(function (err, db) {
    ...
    db.destroy(
      'plugin',
      { name: pluginInfo.name, type: pluginInfo.type },
      function (error) {
        if (error) {
...
PluginManager.prototype.isInstalled = function (pluginInfo, callback) {
  database.getDatabase(function (err, db) {
    ...
    db.retrieve(
      'plugin',
      { type: pluginInfo.type, name: pluginInfo.name },
 ...
 */
PluginManager.prototype.testPluginState = function (
  pluginInfo,
  state,
  callback
) {
...
function loadPackageFile(pluginInfo) {
  if (!pluginInfo.fullPath || 'string' !== typeof pluginInfo.fullPath) {
    throw new Error(
      'Attempted to load version information but fullpath is not set!'
    );
  }
