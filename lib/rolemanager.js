...
var RoleManager = {
    ...
    database.getDatabase(function (err, db) {
      ...
      db.retrieve('role', { name: role.name }, function (error, results) {
        ...
        if (results && results.length) {
          // role exists
          return callback(
            new RoleCreateError('A role with that name already exists!')
          );
        }
  ...
  assignRoleByName: function (roleName, userId, next) {
    ...
    app.rolemanager.retrieveRole({ name: roleName }, function (err, role) {
      ...
      if (!role) {
        return next(
          new Error('Failed to find a role with the name ' + roleName)
        );
      }
  ...
  init: function (app) {
    ...
    rest.get('/role/:id', function (req, res, next) {
      ...
      if ('string' !== typeof id) {
        ...
        return res.json({
          success: false,
          message: 'id param must be an objectid',
        });
      }

      app.rolemanager.retrieveRole(
        { _id: id },
        options,
        function (err, roleRec) {
          if (err) {
            return next(err);
          }
...
function installDefaultRoles(options, cb) {
  ...
  database.getDatabase(function (err, db) {
    ...
    fs.readdir(
      path.join(configuration.serverRoot, 'lib', 'role'),
      function (err, files) {
        async.each(
          files,
          function (file, next) {
            ...
            db.create('role', roledef, function (err, rec) {
              if (err) {
                ...
                logger.log(
                  'warn',
                  'failed to add role to tenant: ' + roledef.name,
                  err
                );
              }
              return next(err);
        ...
        async.each(
          ...
          function (file, callback) {
            ...
            db.update(
              'role',
              { name: role.name },
              { version: role.version, statement: role.statement },
              function (err, rec) {
                if (err) {
                ...
                if (!rec) {
                  logger.log(
                    'warn',
                    '- ' + role.name + " doesn't exist, cannot update",
                    err
                  );
                }
                return callback(err);
