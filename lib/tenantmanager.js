...
exports = module.exports = {
  ...
  createTenantFilesystem: function (tenant, callback) {
    ...
    fs.mkdirs(
      path.join(configuration.tempDir, tenant._id.toString()),
      function (err) {
        if (err) {
          logger.log('error', err);
          return callback(err);
        }
        var tenantFrameworkDir = path.join(
  ...
  createTenant: function (tenant, callback) {
    ...
    database.getDatabase(function (err, db) {
      ...
      if (tenant.database.dbName.length > 64) {
        return callback(
          new TenantCreateError(
            'Database Name must be a maximum of 64 characters long'
          )
        );
      }
