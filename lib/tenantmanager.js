...
exports = module.exports = {
  ...
  createTenantFilesystem: function(tenant, callback) {
    logger.log('info', 'Copying Adapt framework into place for new tenant ' + tenant.name + '(' + tenant._id.toString() + ')');

    fs.mkdir(path.join(TEMP_DIR, tenant._id.toString()), function (err) {
        if (err) {
          logger.log('error', err);
          callback(err);
        }

        logger.log('info', 'Copying Adapt framework into place for this tenant');

        ncp(path.join(process.cwd(), FRAMEWORK_DIR), path.join(TEMP_DIR, tenant._id.toString(), FRAMEWORK_DIR), function (err) {
          if (err) {
            logger.log('error', err);
            callback(err);
          } else {
        ...
        if (error) {
          ...
          db.create('tenant', tenant, function (error, result) {
            ...
            if (error) {
              ...
              _this.createTenantFilesystem(result, function(error) {
                if (error) {
                  callback(error);
                } else {
