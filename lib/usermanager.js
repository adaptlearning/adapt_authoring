...
exports = module.exports = {
   ...
   * @param string emailToCheck - email address as a string
   * @param {function} callback - function of the form function (error)
   */
          ...
          usermanager.checkEmailExists({email:emailParam}, function(err, checkRes){
            if( err ){
              ...
              usermanager.createUserPasswordReset({email:emailParam,token:token.token,tokenCreated: token.expDate },function(err, res){
                ...
                if (res) {
                  // Send Link to User  
                  var subject = 'Mail from Nodemailer';
                  var text= 'Testlink: ' + 'http://' + host + '/#user/reset/' + token.token;
                  app.mailer.send(emailParam, subject, text, function (error, info){
