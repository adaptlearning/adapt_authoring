...
exports = module.exports = {
  ...
  /**
    * handle a users unshared courses on delete
  */
  handleUserCoursesOnDelete: function(userId, courseOption, done) {
    ...
    courseContent.retrieve({ createdBy: userId, _isShared: false }, {}, function (err, results) {

      switch (courseOption) {
        case 'transfer':
          var currentUser = this.getCurrentUser();
          async.each(results, function(model, cb) {
            courseContent.update({ _id: model._id }, { createdBy: currentUser._id }, cb);
          }, done);
          break;
        case 'delete':
          async.each(results, function(model, cb) {
        ...
        case 'share':
          async.each(results, function(model, cb) {
            courseContent.update({ _id: model._id }, { _isShared: true }, cb);
          }, done);
          break;
        default:
          done(null);
   ...
   * @return
  */
  checkPasswordPolicy: function(password, search, done) {
    const errors = [];
    const config = configuration.getConfig('passwordPolicy');
    const map = {
      digits: {
        regex: /\d/g,
        error: app.polyglot.t('app.validationpassworddigits', { length: config.digits })
      },
      lowercaseLetters: {
        regex: /[a-z]/g,
        error: app.polyglot.t('app.validationpasswordlowercase', { length: config.lowercaseLetters })
      },
      uppercaseLetters: {
        regex: /[A-Z]/g,
        error: app.polyglot.t('app.validationpassworduppercase', { length: config.lowercaseLetters })
      },
      nonAlphaNumeric: {
        regex: /[!#@?%&$<>]/g,
        error: app.polyglot.t('app.validationpasswordspecial', { length: config.lowercaseLetters })
      }
    }

    this.retrieveUser(search, function(error, user) {
      if (password.length < config.length) {
        errors.push(app.polyglot.t('app.validationpasswordlength', { length: config.length }));
      }

      if (config.notIncludeNames && user) {
        const userNameRegex = new RegExp([user.email.split('@')[0], user.firstName, user.lastName].join('|'),'gi');
        if (userNameRegex.test(password)) {
          errors.push(app.polyglot.t('app.validationpasswordnames'));
        }
      }
      ...
      if (errors.length) return done(new PasswordValidationError(errors.join('\n')));
      return done(null);
    });
  },
  ...
  init: function (app) {
    ...
    rest.post('/user/resetpassword', function(req, res, next) {
      ...
      self.hasPermission('update', user, '/api/user/resetpassword', function(err, isAllowed) {
        ...
        exports.checkPasswordPolicy(delta.password, {email: delta.email}, function(validationError) {
          if (validationError) {
            res.statusCode = 400;
          ...
          auth.hashPassword(delta.password, function(err, hash) {
            ...
            self.updateUser({email: delta.email}, delta, function(err) {
              ...
              app.mailer.send(delta.email, subject, body, { name: 'emails/passwordReset.hbs' }, function(error) {
                res.statusCode = 200;
    ...
    rest.put('/user/me', function (req, res, next) {
      ...
      if (delta._isNewPassword) {
        ...
        exports.checkPasswordPolicy(delta.password, {email: user.email}, function(validationError) {
          if (validationError) {
    ...
    rest.post('/user', function (req, res, next) {
      ...
      self.createNewUser(userData, function (err, result) {
        if (err && err instanceof PasswordValidationError) {
