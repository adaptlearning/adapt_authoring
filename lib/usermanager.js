...
exports = module.exports = {
   ...
   * 
   */
  findOrCreateUser: function(search, authType, profile, callback) {
    ...
    if (!search.hasOwnProperty('_id') && !search.hasOwnProperty('idNumber') && !search.hasOwnProperty('email')) {
      return callback(new Error("You must specify one of '_id', 'idNumber' or 'email'"));
    }
    ...
    self.retrieveUser(search, function(err, user) {
      ...
      if (user) {
        ...
        // TODO: What about roles/tenants?  
        // Create the user.
        self.createUser(profile, function(err, result) {
          if (err) {
  ...
  serializeUser: function (data, cb) {
    ...
    if (data._tenantId == '000000000000000000000000') { 
