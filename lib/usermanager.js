...
exports = module.exports = {
  ...
  serializeUser: function (data, cb) {
    if ('0' === data.tenant) { // no tenant for user, default to master
      return cb(null, { _id: data._id, email: data.email, tenant: { name: 'default', isMaster: true, database: { dbName: configuration.getConfig('dbName') } } } );
    }
    ...
    tenantmanager.retrieveTenant({ _id : data._tenantId }, function (error, tenant) {
      ...
      cb(null, { _id: data._id, email: data.email, tenant: { _id: tenant._id, name: tenant.name, isMaster: tenant.isMaster } });
    });
  },
  ...
  logAccess: function(user, callback) {
    ...
    this.updateUser({email: user.email}, delta, function(error) {
      if (error) {
        return callback(error);
      }
  ...
  init: function (app) {
    ...
    rest.post('/user/resetpassword', function (req, res, next) {
      ...
      // TODO - Verify the current user has rights
      // HACK -- Allowing @learningpool.com email addresses for now
      if (user.email.indexOf('@learningpool.com') > -1) {
        // Sanatize the input
