...
exports = module.exports = {
  ...
  hasPermission: function (action, user, resource, next) {
    var resourceString = permissions.buildResourceString(
      user.tenant._id,
      resource
    );
  ...
  createNewUser: function (user, callback) {
    ...
    auth.getAuthPlugin(configuration.getConfig('auth'), function (err, plugin) {
      ...
      plugin.internalRegisterUser(false, user, function (error, result) {
        ...
        app.rolemanager.assignRoleByName(
          'Authenticated User',
          result._id,
  ...
  findOrCreateUser: function (search, authType, profile, next) {
    ...
    if (
      !search.hasOwnProperty('_id') &&
      !search.hasOwnProperty('idNumber') &&
      !search.hasOwnProperty('email')
    ) {
      return next(
        new Error("You must specify one of '_id', 'idNumber' or 'email'")
      );
    }
        ...
        async.waterfall(
          [
            ...
            function (newUser, callback) {
              ...
              if (
                !profile.hasOwnProperty('_tenantId') ||
                profile._tenantId == UNDEFINED_TENANT_ID
              ) {
                ...
                tenantmanager.createTenant(
                  ...
                  function (err, tenant) {
                    if (err) {
                      logger.log(
                        'error',
                        'Failed to create new tenant for user: ',
                        profile
                      );
                      return callback(err);
                    }
    ...
    database.getDatabase(function (err, db) {
      db.retrieve('user', search, options, function (error, results) {
        ...
        if (results && results.length > 0) {
          ...
          return callback(
            new Error(
              'user search expected a single result but returned ' +
                results.length +
                ' results'
            )
          );
          ...
          database.getDatabase(
            function (error, db) {
              ...
              async.parallel(
                [
                  function (cb) {
                    this.handleUserCoursesOnDelete(user._id, courseOption, cb);
                  }.bind(this),
                  function (cb) {
                    db.destroy('userpasswordreset', { user: user._id }, cb);
                  },
                  function (cb) {
                    db.destroy('user', user, cb);
                  },
                ],
                callback
              );
  ...
  getCurrentUser: function () {
    try {
      // parse the session user to give us a plain object without ObjectIds
    ...
    tenantmanager.retrieveTenant(
      ...
      function (error, tenant) {
        ...
        if (!tenant) {
          logger.log(
            'error',
            `Unable to retrieve tenant ${data._tenantId} for user ${data._id}`
          );
  ...
  createUserPasswordReset: function (userReset, next) {
    ...
    this.retrieveUser(
      { email: userReset.email, auth: 'local' },
      function (error, user) {
        if (error) {
          return next(error);
        } else if (user) {
                  ...
                  db.update(
                    ...
                    function (error, result) {
                      if (error) {
                        logger.log(
                          'error',
                          'Failed to update user password reset: ',
                          user
                        );
                        return next(error);
                      } else {
                  ...
                  db.create(
                    ...
                    function (error, result) {
                      if (error) {
                        logger.log(
                          'error',
                          'Failed to create user password reset: ',
                          user
                        );
                        return next(error);
                      } else {
    ...
    database.getDatabase(function (err, db) {
      db.retrieve('UserPasswordReset', search, function (error, results) {
        ...
        if (results.length > 1) {
          return callback(
            new Error(
              'User password reset search expected a single result but returned ' +
                results.length +
                ' results'
            )
          );
  ...
  resetUserPassword: function (user, callback) {
    ...
    this.retrieveUserPasswordReset(
      { token: user.token },
      function (err, resetRequest) {
        if (err) {
          // Token is too old
  ...
  init: function (app) {
    ...
    rest.get('/user', (req, res, next) => {
      ...
      this.retrieveUsers(
        query,
        { populate: { roles: 'name', _tenantId: 'name' } },
        function (err, users) {
        ...
        function (err, isAllowed) {
          ...
          auth.hashPassword(delta.password, function (err, hash) {
            ...
            self.updateUser({ email: email }, delta, function (err) {
              ...
              logger.log(
                'info',
                'Password reset by administrator for ' + email
              );

              // try to send a notification mail, no biggie if it fails
              var subject = app.polyglot.t(
                'app.emailadminpasswordchangesubject'
              );
              var body = app.polyglot.t('app.emailadminpasswordchangebody', {
      ...
      self.hasPermission(
        ...
        function (err, isAllowed) {
          ...
          self.retrieveUser({ _id: user._id }, {}, function (err, userObject) {
            ...
            var body = app.polyglot.t('app.emailinvitebody', {
              inviter: inviterDetails,
              rootUrl: configuration.getConfig('rootUrl'),
              resetUrl: configuration.getConfig('rootUrl') + '/#user/forgot',
            });
            app.mailer.send(
              req.body.email,
              subject,
              body,
              { name: 'emails/invite.hbs' },
              function (error) {
                if (error) {
    ...
    rest.put('/user/me', function (req, res, next) {
      ...
      if (user._id !== delta._id || user.email !== delta.email_prev) {
        return res
          .status(400)
          .json({ success: false, message: 'Trying to update wrong user' });
      }
      ...
      if (delta._isNewPassword) {
        ...
        logger.log(
          'info',
          'Resetting password for ' + delta.email + '(' + user._id + ')'
        );
    ...
    rest.delete('/user/:id', function (req, res, next) {
      ...
      self.retrieveUser({ _id: id }, function (error, user) {
        var resourceStr = permissions.buildResourceString(
          user._tenantId,
          '/user/' + user._id
        );
    ...
    rest.get('/userpasswordreset/:token', function (req, res, next) {
      ...
      if (token) {
        self.retrieveUserPasswordReset(
          { token: token },
          function (err, usrReset) {
            res.statusCode = 200;
            if (err) {
              return res.json(err);
            }
