...
exports = module.exports = {
  ...
  createNewUser: function(user, callback) {
    ...
    auth.getAuthPlugin(configuration.getConfig('auth'), function (err, plugin) {
      ...
      plugin.internalRegisterUser(user, function(error, userRecord) {
        if (error) {
          return callback(error);
        }
        app.rolemanager.assignRoleByName('Authenticated User', userRecord._id, function (error, roleRecord) {
          if (error) {
            return callback(error);
          }
          // return the updated
          userRecord.roles = roleRecord.roles;
  ...
  init: function (app) {
      ...
      self.hasPermission('update', user, '/api/user/resetpassword', function(err, isAllowed) {
        ...
        auth.hashPassword(delta.password, function(err, hash) {
          ...
          self.updateUser({email: delta.email}, delta, function(err) {
            ...
            // attempt to send a notification mail
    ...
    rest.post('/user', function (req, res, next) {
      ...
      var noPass = userData.password === undefined;
      if(noPass) {
        // create a temporary password
        userData.password = generatePassword();
      }
      self.createNewUser(userData, function (error, userRecord) {
        if (error) {
          res.status(500);
          return next(error);
        }
        if(noPass) {
          // create reset token and send email prompting user to set password
          auth.getAuthPlugin(configuration.getConfig('auth'), function(error, authPlugin) {
            ...
            authPlugin.internalGenerateResetToken(user, function (error, userToken) {
              if (error) {
                res.status(500);
                return next(error);
              }
              // send mail
              var subject = app.polyglot.t('app.emailsetpasswordsubject');
              var body = app.polyglot.t('app.emailsetpasswordbody', { rootUrl: configuration.getConfig('rootUrl'), data: userToken.token });
              var templateData = {
                name: 'passwordReset',
                rootUrl: configuration.getConfig('rootUrl'),
                token: userToken.token,
                type: 'set',
                buttonLabel: app.polyglot.t('app.setpassword')
              };
              app.mailer.send(user.email, subject, body, templateData, function(error) {
                if(error) {
                  return res.status(500).json({ error: error });
                }
                logger.log('info', 'Password reset sent for ' + user.email + ' from ' + user.ipAddress);
