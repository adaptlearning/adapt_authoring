...
exports = module.exports = {
  ...
  handleUserCoursesOnDelete: function(userId, courseOption, done) {
    ...
    courseContent.retrieve({ createdBy: userId }, {}, function (err, results) {
      var currentUser = this.getCurrentUser();
      async.eachSeries(results, function(model, cb) {
        ...
        courseContent.update({ _id: model._id }, { createdBy: currentUser._id }, function (err, course) {
          if (!course._isShared && course._shareWithUsers.length === 0) {
            // Delete private courses if this option was set
            if (courseOption === 'delete') {
              courseContent.destroy({ _id: course._id }, false, cb);
              return;
            }
            // Share private courses with everyone if this option was set, remove the current user from the 'shared with' list if applicable
            if (courseOption === 'share') {
              courseContent.update({ _id: course._id }, { _isShared: true, _shareWithUsers: course._shareWithUsers.pull(currentUser._id) }, cb);
              return;
            }
          }
          // Remove the current user from the 'shared with' list if applicable
          courseContent.update({ _id: course._id }, { _shareWithUsers: course._shareWithUsers.pull(currentUser._id) }, cb);
        }.bind(this));
      }.bind(this), done);
    }.bind(this));
  },
