...
exports = module.exports = {
  ...
  serializeUser: function (data, cb) {
    if ('0' === data.tenant) { // no tenant for user, default to master
      return cb(null, { _id: data._id, tenant: { name: 'default', database: { dbName: configuration.getConfig('dbName') } } } );
    }
    ...
    tenantmanager.retrieveTenant({ _id : data.tenant }, function (error, tenant) {
      ...
      cb(null, { _id: data._id, tenant: tenant });
    });
  },
  ...
  init: function (app) {
    ...
    app.once('serverStarted', function(){
      ...
      rest.get('/user/me', function (req, res, next) {
        var usr = req.session.passport.user;//@todo : why didn't this work?? instance.getCurrentUser();
