        ...
        courseContent.update({ _id: model._id }, { createdBy: currentUser._id }, function (err, course) {
          if (!course._isShared && course._shareWithUsers.length === 0) {
            ...
            if (courseOption === 'share') {
              courseContent.update({ _id: course._id }, { _isShared: true, _shareWithUsers: course._shareWithUsers.filter(item => item !== currentUser._id) }, cb);
              return;
            }
          }
          courseContent.update({ _id: course._id }, { _shareWithUsers: course._shareWithUsers.filter(item => item !== currentUser._id) }, cb);
        }.bind(this));
      }.bind(this), done);
    }.bind(this));
  },
