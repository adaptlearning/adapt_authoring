...
exports = module.exports = {
  ...
  serializeUser: function (data, cb) {
    ...
    tenantmanager.retrieveTenant({ _id : data._tenantId }, function (error, tenant) {
      ...
      cb(null, { _id: data._id, email: data.email, tenant: { _id: tenant._id, name: tenant.name } });
    });
  },
  ...
  init: function (app) {
    ...
    rest.get('/user', function(req, res, next) {
      self.retrieveUsers({}, { populate: { 'roles' : 'name' } }, function(err, users) {
        if (err) {
          return next(err);
        }
    ...
    rest.get('/user/:id', function (req, res, next) {
      ...
      self.retrieveUser({ _id: id }, { populate: { 'roles' : 'name' } }, function (err, usr) {
        if (err) {
          return next(err);
        }
