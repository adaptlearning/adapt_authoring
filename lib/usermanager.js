...
exports = module.exports = {
  ...
  */
  validatePassword: function(password) {
    ...
    if (errors.length) return new PasswordValidationError(errors.join('\n'));
    return null;
  },
  ...
  init: function (app) {
    ...
    rest.post('/user/resetpassword', function(req, res, next) {
      ...
      self.hasPermission('update', user, '/api/user/resetpassword', function(err, isAllowed) {
        ...
        const validationError = exports.validatePassword(delta.password);
        if (validationError) {
          res.statusCode = 400;
          return res.json({success: false, message: validationError.toString()});
        }
