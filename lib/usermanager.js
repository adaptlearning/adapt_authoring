...
exports = module.exports = {
  ...
  hasPermission: function(action, user, next) {
    permissions = require('./permissions');
    var resource = permissions.buildResourceString(user.tenant._id, '/api/user/resetpassword');
  ...
  init: function (app) {
    ...
    rest.post('/user/resetpassword', function(req, res, next) {
      ...
      self.hasPermission('update', user, function(err, isAllowed) {
        if (err) {
          res.statusCode = 500;
          return res.json(err);
        }
