...
exports = module.exports = {
  ...
  deleteUserPasswordReset: function (user, callback) {    
    ...
    this.retrieveUserPasswordReset({token:user.token}, function (error, result) {
      if (error) {        
        callback(error);
      } else if (result) {        
        database.getDatabase(function(err,db){
  ...
  resetUserPassword: function (user, callback) {
    ...
    this.retrieveUserPasswordReset({token:user.token}, function (err, resetRequest) {
      ...
      if (resetRequest.tokenCreated.getTime() > timestampMinAge) {
        // update user record
        usermanager.updateUser({_id:resetRequest.user}, user, function(err) {
          if (err) {            
            return callback(err, false);
          }
          // delete passwordReset record
          usermanager.deleteUserPasswordReset({token:user.token}, function(err, res) {});
  ...
  generateToken: function () {
    // Generate the token          
    return {token: crypto.randomBytes(64).toString('hex'), expDate: new Date()};

  },

  sendTokenMail: function (token, host, email, cback){
 ...
 // create reusable transporter object using SMTP transport
          ...
          var mailOptions = {
              ...
              text: 'Testlink: ' + 'http://' + host + '/#user/reset/' + token
          };
  ...
  checkEmailExists : function(emailToCheck, callback){
    database.getDatabase(function(err,db){
      db.retrieve('user', emailToCheck, function (error, results) {
        if (error) {
          callback(error);
        } else if (results && results.length > 0) {
          if (results.length === 1) {
  ...
  init: function (app) {
    var usermanager = this;
        ...
        if (user) {
          ...
          if (delta._isNewPassword) {
            ...
            auth.hashPassword(delta.password, function(err, hash) {
              ...
              usermanager.updateUser({email: delta.email}, delta, function(err) {
                if (err) {
                  res.statusCode = 500;
                  return res.json(err);
                }
        ...
        if (token) {
          usermanager.retrieveUserPasswordReset({token:token}, function (err, usrReset) {    
            if ( err ){
              ...
              // res.statusCode = 200;
              return res.json({error:false,success:usrReset});
            }
            // res.statusCode = 404;
            return res.json({error:false,success:false});
      ...
      rest.post('/handleResetPassword/', function (req, res, next){
        ...
        auth.hashPassword(req.body.password, function(err, hash) {
          ...
          usermanager.resetUserPassword(req.body, function(err, result){
            if (err) {
              // res.statusCode = 500;
              return res.json({error:err,success:false});  
            }
            if (result) {
              // res.statusCode = 200;
              return res.json({error:false,success:result});  
            }
          });    
      ...
      rest.get('/sendResetLink/:email', function (req, res, next) {
        var emailParam = req.params.email;
        var host = req.headers.host;
        
        if (emailParam) {
          // check if email exists in dB
          usermanager.checkEmailExists({email:emailParam}, function(err, checkRes){
            if( err ){
              ...
              // Email exists
              // Generate ResetToken
              var token = usermanager.generateToken();              

              // save token to db
              usermanager.createUserPasswordReset({email:emailParam,token:token.token,tokenCreated: token.expDate },function(err, res){
                //there was an error creating the password reset
                if (err) {
                  // res.statusCode = 500;
                  return res.json({error:err,success:false});  
                }
                //the password reset was created
                if (res) {
                  // Send Link to User                            
                  usermanager.sendTokenMail(token.token, host, checkRes.email, function(error,info){                                  
                    return res.json({error:error,info:info});
                  });
                  // res.statusCode = 200;
                  return res.json({error:false,success:res});  
                }
              });
            };
          });
