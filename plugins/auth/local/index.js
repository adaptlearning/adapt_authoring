...
LocalAuth.prototype.internalRegisterUser = function (
  ...
  if (retypePasswordRequired) {
    if (!user.email || !user.password || !user.retypePassword) {
      return cb(
        new auth.errors.UserRegistrationError(
          'email, password and retyped password are required!'
        )
      );
    }
  ...
  if (!user.email || !user.password) {
    return cb(
      new auth.errors.UserRegistrationError('email and password are required!')
    );
  }
...
LocalAuth.prototype.resetPassword = function (req, res, next) {
  ...
  usermanager.retrieveUserPasswordReset(
    ...
    function (error, usrReset) {
      ...
      self.internalResetPassword(
        { id: usrReset.user, password: req.body.password },
        function (error, user) {
          if (error) {
            logger.log('error', error);
            return res.status(500).end();
...
LocalAuth.prototype.internalResetPassword = function (user, next) {
  if (!user.id || !user.password) {
    return next(
      new auth.errors.UserResetPasswordError(
  ...
  auth.hashPassword(user.password, function (error, hash) {
    ...
    usermanager.updateUser(
      { _id: user.id },
      { password: hash, failedLoginCount: 0 },
      function (err) {
        if (error) {
          return next(error);
        }

        usermanager.deleteUserPasswordReset(
          { user: user.id },
          function (error, user) {
            if (error) {
              return next(error);
...
LocalAuth.prototype.generateResetToken = function (req, res, next) {
  ...
  usermanager.retrieveUser(
    { email: req.body.email, auth: 'local' },
    function (error, userRecord) {
      ...
      if (userRecord) {
        ...
        self.internalGenerateResetToken(user, function (error, userToken) {
          ...
          if (!userToken) {
            return next(
              new auth.errors.UserGenerateTokenError(
                'In generateResetToken and user object is not set!'
              )
            );
          }

          var subject = app.polyglot.t('app.emailforgottenpasswordsubject');
          var body = app.polyglot.t('app.emailforgottenpasswordbody', {
          ...
          app.mailer.send(
            ...
            function (error) {
              ...
              logger.log(
                'info',
                'Password reset for ' + user.email + ' from ' + user.ipAddress
              );
