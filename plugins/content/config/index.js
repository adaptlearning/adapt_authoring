 ...
 */
ConfigContent.prototype.hasPermission = function (
  action,
  userId,
  tenantId,
  contentItem,
  ...
  app.once('serverStarted', function (server) {
    ...
    contentmanager.addContentHook(
      ...
      function (course, next) {
        ...
        contentmanager.create.apply(contentmanager, [
          'config',
          configObj,
          function (err, results) {
            ...
            if (err) {
              logger.log(
                'error',
                'config hook failed on course creation: ' + err.message
              );
            }
            next(null, course);
          },
...
ConfigContent.prototype.retrieve = function (search, options, next) {
  ...
  if (search._id) {
    ...
    delete search._id;
  } else if (search._configId) {
    // allow override to search on configId instead
    search._id = search._configId;
  }

  ContentPlugin.prototype.retrieve.call(
    this,
    search,
    options,
    function (err, records) {
      ...
      if (!records || records.length == 0) {
        return next(
          new Error(
            `Unable to retrieve ${modelName} for ${JSON.stringify(search)}`
          )
        );
      }
