 ...
 */
CourseAssetContent.prototype.hasPermission = function (
  action,
  userId,
  tenantId,
  contentItem,
...
function initialize() {
  ...
  ['course', 'contentobject', 'article', 'block', 'component'].forEach(
              ...
              if (items && items.length == 1) {
                  ...
                  [
                    function (callback) {
                      if (contentType !== 'contentobject') {
                        return callback(
                          null,
                          'Not processing a content object'
                        );
                      }
                      db.retrieve(
                        'article',
                        {
                          _courseId: itemForDeletion._courseId,
                          _parentId: itemForDeletion._id,
                        },
                        function (err, articles) {
                          if (err) {
                            return callback(
                              err,
                              'Unable to retrieve child articles of contentobject ' +
                                itemForDeletion._id
                            );
                          }
                          var parentIds = [];
                          ...
                          async.each(
                              ...
                              db.retrieve(
                                ...
                                function (err, blocks) {
                                  if (err) {
                                    return callback(
                                      err,
                                      'Unable to retrieve child blocks of article ' +
                                        article._id
                                    );
                                  }
                            ...
                            function (error) {
                              if (error) {
                                ...
                                callback(
                                  null,
                                  'Child content objects added to criteria object'
                                );
                              }
                            }
                          );
                ...
                logger.log(
                  ...
                  'Unexpected number of ' +
                    contentType +
                    ' items found in courseasset pre-delete hook'
                );
                next(null, data);
              }
            });
