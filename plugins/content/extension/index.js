        ...
        async.each(
          ...
          function (doc, nextItem) {
            ...
            Object.keys(enabledExtensions).forEach(function (key) {
              if (
                enabledExtensions[key]._id.toString() ===
                oldPlugin._id.toString()
              ) {
      ...
      function (callback) {
        if (contentType == 'component') {
          ...
          database.getDatabase(function (error, db) {
            ...
            db.retrieve(
              'componenttype',
              { component: contentData._component },
                        ...
                        if (!courseGlobals._components[key]) {
                          ...
                          for (var prop in componentType.globals) {
                            if (componentType.globals.hasOwnProperty(prop)) {
                              componentGlobals[prop] =
                                componentType.globals[prop].default;
                            }
                          }
        ...
        getEnabledExtensions(
          ...
          function (error, extensions) {
            ...
            extensions.forEach(function (extensionItem) {
              if (
                extensionItem.properties.hasOwnProperty('pluginLocations') &&
                extensionItem.properties.pluginLocations.properties[contentType]
              ) {
                var schema =
                  extensionItem.properties.pluginLocations.properties[
                    contentType
                  ].properties; // yeesh
                var generatedObject = helpers.schemaToObject(
                  schema,
                  extensionItem.name,
                ...
                contentData._extensions = _.defaults(
...
function toggleExtensions(courseId, action, extensions, cb) {
  ...
  database.getDatabase(function (err, db) {
    ...
    var updateComponentItems = function (
      tenantDb,
      componentType,
      schema,
      extensionItem,
      ...
      var criteria =
        'course' == componentType ? { _id: courseId } : { _courseId: courseId };
      tenantDb.retrieve(
        componentType,
        criteria,
        { fields: '_id _extensions _enabledExtensions' },
        function (err, results) {
          if (err) {
            return cb(err);
          }
    ...
    db.retrieve(
      ...
      function (err, results) {
        ...
        database.getDatabase(function (err, tenantDb) {
          ...
          async.eachSeries(
            results,
            function (extensionItem, nextItem) {
              var locations =
                extensionItem.properties.pluginLocations.properties;
                      ...
                      if (!courseGlobals._extensions[key]) {
                        ...
                        for (var prop in extensionItem.globals) {
                          if (extensionItem.globals.hasOwnProperty(prop)) {
                            extensionGlobals[prop] =
                              extensionItem.globals[prop].default;
                          }
                        }
                      ...
                      function (err, doc) {
                        if (!err) {
                          async.eachSeries(
                            ...
                            function (key, nextLocation) {
                              updateComponentItems(
                                tenantDb,
                                key,
                                locations[key].properties,
                                extensionItem,
                                nextLocation
                              );
                            },
...
Extension.prototype.getUses = function (callback, id) {
  database.getDatabase(function (err, db) {
    ...
    db.retrieve('extensiontype', { _id: id }, function (err, extensiontypes) {
      ...
      const search = {};
      search['_enabledExtensions.' + extensiontypes[0].extension] = {
        $exists: true,
      };
...
function initialize() {
      ...
      disableExtensions(
        ...
        function (error) {
          if (error) {
            ...
            return res
              .status(error instanceof ContentTypeError ? 400 : 500)
  ...
  ['contentobject', 'article', 'block', 'component'].forEach(function (
    contentType
  ) {
    app.contentmanager.addContentHook(
      'create',
      contentType,
      contentCreationHook.bind(null, contentType)
    );
  });
  ...
  ['component'].forEach(function (contentType) {
    app.contentmanager.addContentHook(
      'destroy',
      contentType,
      contentDeletionHook.bind(null, contentType)
    );
  });
}
