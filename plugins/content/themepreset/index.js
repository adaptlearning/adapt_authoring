...
ThemePresetContent.prototype.hasPermission = function (
  action,
  userId,
  tenantId,
  contentItem,
...
ThemePresetContent.prototype.updatePreset = function (
  presetId,
  courseId,
  res,
  next
      ...
      app.contentmanager.update(
        ...
        function (err) {
          ...
          app.contentmanager.update(
            'course',
            { _id: courseId },
            { _courseId: courseId, themeSettings: null },
            function (err) {
              if (err) return next(err);
...
function initialize() {
  ...
  app.once('serverStarted', function (server) {
    app.rest.post(
      '/themepreset/:presetid/makeitso/:courseid',
      function (req, res, next) {
        var presetId = req.params.presetid;
        var courseId = req.params.courseid;

        if (presetId === 'null') {
          return new ThemePresetContent().updatePreset(
            presetId,
            courseId,
            res,
            next
