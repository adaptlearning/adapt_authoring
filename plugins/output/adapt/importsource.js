...
function ImportSource(req, done) {
  ...
  async.series(
    ...
    (importErr, result) => {
      // cleanup should run regardless of import success
      helpers.cleanUpImport(cleanupDirs, (cleanUpErr) =>
        done(importErr || cleanUpErr)
      );
    }
  );
  ...
  function installPlugins(done) {
    async.each(
      ...
      function (pluginData, donePluginIterator) {
        // Ignore white & amber, nothing to install
        if (
          pluginData.name in details.pluginVersions.white ||
  ...
  function addAssets(done) {
    async.eachSeries(
      assetFolders,
      function iterator(assetDir, doneAssetFolder) {
        var assetDirPath = path.join(COURSE_JSON_PATH, COURSE_LANG, assetDir);

        if (!fs.existsSync(assetDirPath)) {
          logger.log(
            'error',
            'Framework import error. Cannot find folder: ' + assetDirPath
          );
        ...
        var assetsJsonFilename = path.join(
          COURSE_JSON_PATH,
          COURSE_LANG,
        ...
        glob(assetsGlob, function (error, assets) {
          ...
          async.eachSeries(
            assets,
            function iterator(assetPath, doneAsset) {
              if (error) {
              ...
              if (!assetJson)
                return helpers.importAsset(fileMeta, metadata, doneAsset);

              addAssetTags(assetJson, function (error, assetTags) {
                const warn = (error) =>
                  logger.log('warn', `Failed to create asset tag ${error}`);
  ...
  function addAssetTags(assetJson, cb) {
    ...
    assetJson.tags.forEach(function (tag) {
      ...
      app.contentmanager.getContentPlugin('tag', function (error, plugin) {
        ...
        plugin.create({ title: tagTitle }, function (error, record) {
          // @note retrieves if tag already exists
          if (error) return cb(tagTitle.concat(' ', error));
    ...
    async.series(
      [
        function enableExtensions(cb) {
          ...
          async.eachSeries(
            plugindata.pluginIncludes,
            function (pluginData, doneItemIterator) {
              switch (pluginData.type) {
                case 'extension':
                  fs.readJson(
                    path.join(pluginData.location, Constants.Filenames.Bower),
                    function (error, extensionJson) {
                      if (error) return cb(error);
                      includeExtensions[extensionJson.extension] =
                        metadata.extensionMap[extensionJson.extension];
                      doneItemIterator();
                    }
                  );
                  break;
              ...
              switch (type) {
                ...
                case 'contentobject': {
                  ...
                  Object.keys(byParent).forEach((id) => {
                    byParent[id].forEach(
                      (item, index) => (item._sortOrder = index + 1)
                    );
                  });
                  var groups = _.groupBy(contentJson, '_type');
                  var sortedSections = helpers.sortContentObjects(
                    groups.menu,
                    origCourseId,
                    []
                  );
                ...
                function (item, cb3) {
                  createContentItem(type, item, function (error, contentRec) {
                    ...
                    if (!contentRec || !contentRec._id) {
                      logger.log(
                        'warn',
                        'Failed to create map for ' + item._id
                      );
                      return cb3();
                    }
        ...
        async function populateGlobals() {
          ...
          await Promise.all(
            plugindata.pluginIncludes.map(async ({ type, name }) => {
              ...
              courseGlobals[typeKey][pluginKey] = _.defaults(
                courseGlobals[typeKey][pluginKey],
                schemaDefaults
              );
            })
          );
        ...
        function checkDetachedContent(cb) {
          ...
          const errorMsg = Object.keys(groups).reduce(
            function (errorString, group) {
  ...
  function transformContent(type, originalData) {
    return new Promise(async (resolve, reject) => {
      ...
      if (type === 'course') {
        ...
        try {
          var contents = await fs.readFile(
            path.join(
              COURSE_ROOT_FOLDER,
              Constants.Folders.Source,
              Constants.Folders.Theme,
              plugindata.theme[0]._theme,
              'less',
      ...
      var customProps = _.pick(
        data,
        _.difference(Object.keys(data), genericPropKeys)
      );
      ...
      plugindata.pluginTypes.forEach(function (typeData) {
        ...
        var pluginKeys = _.intersection(
          Object.keys(customProps),
          Object.keys(pluginLocations[typeData.type])
        );
    ...
    async.series(
      [
        function transform(cb) {
          transformContent(type, originalData)
        ...
        function updateAssetData(cb) {
          var newAssetPath =
            Constants.Folders.Course + '/' + Constants.Folders.Assets; // always use '/' for paths in content
  ...
  function createCourseAssets(type, contentData, cb) {
    ...
    switch (type) {
      case 'course':
        assetData._courseId =
          assetData._contentTypeId =
          assetData._contentTypeParentId =
            contentData._id;
        break;
    ...
    async.each(
      ...
      function (data, callback) {
        ...
        var matchingAssetId = _.findKey(
          metadata.assetNameMap,
          (value) => value === assetBaseName
        );

        if (!matchingAssetId) return callback();

        app.assetmanager.retrieveAsset(
          { _id: matchingAssetId },
          function gotAsset(error, results) {
            if (error) {
              logger.log('error', error);
              ...
              function (error, plugin) {
                ...
                plugin.create(assetData, function (error, assetRecord) {
                  if (error)
                    logger.log(
                      'warn',
                      `Failed to create courseasset ${type} ${
                        assetRecord || ''
                      } ${error}`
                    );
                  callback(error);
