...
function publishCourse(courseId, mode, request, response, next) {
  ...
  const FRAMEWORK_ROOT_FOLDER = path.join(
  ...
  const COURSES_FOLDER = path.join(
    FRAMEWORK_ROOT_FOLDER,
  ...
  const getGruntFatalError = (stdout) => {
    ...
    return stdout.substring(
      indexStart,
      indexEnd !== -1 ? indexEnd : stdout.length
    );
  };
  ...
  async.waterfall(
    [
      ...
      //
      function (callback) {
        var temporaryThemeFolder = path.join(
          SRC_FOLDER,
        ...
        self.applyTheme(
          ...
          courseId,
          outputJson,
          temporaryThemeFolder,
          function (err, appliedThemeName) {
            if (err) {
              return callback(err);
            }

            self.writeCustomStyle(
              tenantId,
              courseId,
              temporaryThemeFolder,
              function (err) {
      ...
      function (callback) {
        self.buildFlagExists(
          path.join(BUILD_FOLDER, Constants.Filenames.Rebuild),
          function (err, exists) {
            if (err) {
              return callback(err);
            }
      ...
      function (callback) {
        var temporaryMenuFolder = path.join(
          SRC_FOLDER,
      ...
      function (callback) {
        fs.exists(
          path.join(BUILD_FOLDER, Constants.Filenames.Main),
          function (exists) {
            ...
            var args = [];
            var outputFolder = COURSE_FOLDER.replace(
              FRAMEWORK_ROOT_FOLDER + path.sep,
              ''
            );

            // Append the 'build' folder to later versions of the framework
            if (
              semver.gte(semver.clean(frameworkVersion), semver.clean('2.0.0'))
            ) {
            ...
            logger.log(
              'info',
              'grunt server-build:' + buildMode + ' ' + args.join(' ')
            );

            child = exec(
              'grunt server-build:' + buildMode + ' ' + args.join(' '),
              { cwd: path.join(FRAMEWORK_ROOT_FOLDER) },
      ...
      function (callback) {
        const configPath = path.join(
          BUILD_FOLDER,
          Constants.Folders.Course,
          Constants.CourseCollections.config.filename
        );
