...
server.post('/importsourcecheck', function (request, response, next) {
  app.outputmanager.getOutputPlugin(
    app.configuration.getConfig('outputPlugin'),
    ...
    function (error, plugin) {
      ...
      plugin.importsource(request, function (error) {
        if (!error) {
          ...
          return response
            .status(200)
            .send({ body: app.polyglot.t('app.importcoursesuccess') });
        }
...
server.post('/import', function (request, response, next) {
  var outputplugin = app.outputmanager.getOutputPlugin(
    app.configuration.getConfig('outputPlugin'),
    function (error, plugin) {
      if (error) {
        logger.log('error', error);
