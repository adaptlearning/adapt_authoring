...
it('should be able to write a file to disk', function (done) {
  localfs.putFileContents(
    ...
    function (error) {
      ...
      fs.readFile(
        localfs.resolvePath(testData.filename),
        function (error, fsContents) {
          should.not.exist(error);
          fsContents
            .toString()
            .should.equal(testData.fileContents, 'Unexpected file contents');
          done();
        }
      );
    }
...
it('should be able to move a file', function (done) {
  localfs.moveFile(
    ...
    function (error) {
      should.not.exist(error);
      fs.readFile(
        localfs.resolvePath(path.join(testData.dirname, testData.filename)),
        function (error, fsContents) {
          should.not.exist(error);
          fsContents
            .toString()
...
it('should be able to list the contents of a directory', function (done) {
  localfs.getDirectoryListing(testData.dirname, function (error, contents) {
    should.not.exist(error);
    fs.readdir(
      localfs.resolvePath(testData.dirname),
      function (error, fsContents) {
        should.not.exist(error);
