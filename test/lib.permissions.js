  ...
  usermanager.createUser(user, function (error, userRec) {
    ...
    permissions.createPolicy(user._id, function (error, policy) {
      ...
      permissions.addStatement(
        policy,
        'delete',
        permissions.buildResourceString(user._tenantId, '/foos/*'),
        'allow',
        function (error) {
          if (error) return done(error);
          return rolemanager.assignRoleByName(
            'Authenticated User',
            user._id,
            done
          );
...
it('should be able to create permissions for a user', function (done) {
  permissions.createPolicy(user._id, function (error, policy) {
    ...
    permissions.addStatement(
      policy,
      'create',
      permissions.buildResourceString(user._tenantId, '/courses'),
      'allow',
      done
    );
  });
...
it('should verify user permissions using a glob-type resource identifier', function (done) {
  permissions.hasPermission(
    user._id,
    'delete',
    permissions.buildResourceString(user._tenantId, '/foos/bars'),
    function (error, allowed) {
      should.not.exist(error);
      allowed.should.be.true;
      done();
